...
\begin{tikzpicture}[
  ...
  gpu/.style={
    rectangle, minimum width=8mm, minimum height=8mm,
    inner sep=1pt, draw=#1, fill=#1!15, line width=0.7pt,
    font=\tiny
  },
]
% Draw the 4x4x2 grid: DP=4 (x), PP=4 (y), TP=2 (z)

% TP dimension (z): 2 layers
% PP dimension (y): 4 stages
% DP dimension (x): 4 replicas

% Bottom layer (TP rank 0)
\foreach \dp in {0,...,3} {
  \foreach \pp in {0,...,3} {
    \node[gpu=figBlue] (b-\dp-\pp) at (\dp*1.8, \pp*1.5, 0)
      {\dp,\pp,0};
  }
}
...
\foreach \dp in {0,...,3} {
  \foreach \pp in {0,...,3} {
    \node[gpu=figRed] (t-\dp-\pp) at (\dp*1.8, \pp*1.5, 1.5)
      {\dp,\pp,1};
  }
}

% TP connections (vertical, within node)
\foreach \dp in {0,...,3} {
  \foreach \pp in {0,...,3} {
    \draw[figGreen, thick, densely dashed]
      (b-\dp-\pp.north) -- (t-\dp-\pp.south);
  }
}

% PP connections (depth, across stages)
\foreach \dp in {0,...,3} {
  \foreach \pp in {0,...,2} {
    \pgfmathtruncatemacro{\ppnext}{\pp+1}
    \draw[figOrange, thick, ->]
      (b-\dp-\pp.north east) -- (b-\dp-\ppnext.south west);
  }
}

% Axis labels
\draw[thick, ->] (-0.5, -0.5, 0) -- (7.5, -0.5, 0)
  node[midway, below, font=\small] {DP (数据并行) $\times 4$};
\draw[thick, ->] (-0.5, -0.5, 0) -- (-0.5, 5.5, 0)
  node[midway, left, font=\small, rotate=35] {PP (流水线并行) $\times 4$};
\draw[thick, ->] (-0.5, -0.5, 0) -- (-0.5, -0.5, 2.2)
  node[midway, left, font=\small] {TP $\times 2$};

% Communication annotations
\node[font=\scriptsize, figGreen, align=center] at (8.5, 2, 1.0)
  {TP: NVLink\\(900 GB/s)};
\node[font=\scriptsize, figOrange, align=center] at (8.5, 2, -0.3)
  {PP: IB/NVLink\\(100--900 GB/s)};
\node[font=\scriptsize, figBlue, align=center] at (3.5, -1.5, 0)
  {DP: AllReduce\\(IB, 100 GB/s)};

\end{tikzpicture}
\caption{
  3D 并行示意图：$\mathrm{DP} \times \mathrm{PP} \times \mathrm{TP} = 4 \times 4 \times 2 = 32$ 块 GPU。
  每个小方块代表一块 GPU，坐标 $(d, p, t)$ 表示
  其在数据并行、流水线并行和张量并行中的位置。
  \textcolor{figGreen}{绿色虚线}：TP 通信（NVLink，最高带宽）。
  \textcolor{figOrange}{橙色箭头}：PP 通信（激活值和梯度在流水线阶段间传递）。
  DP 通信（AllReduce）发生在沿 $x$ 轴的同位置 GPU 之间。
  通信带宽需求从高到低：TP $>$ PP $>$ DP。
}
