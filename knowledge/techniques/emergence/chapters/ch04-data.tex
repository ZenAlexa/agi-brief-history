...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{FineWeb-2}（HuggingFace，2024 年 12 月首发）：
        FineWeb 的多语言继任者。
        截至 2026 年初稳定在
        \textbf{20TB / 5B 文档 / 1868 个 language-script 对}
        （其中 1226 对超过 100MB），
        覆盖 96 个 Common Crawl 快照（2013 年夏 -- 2024 年 4 月）。
        Hugging Face 的数据 viewer 上次更新于 2025 年 10 月 27 日，
        共 4.48B 行。
        FineWeb-2 的论文 arXiv:2506.20920 已被 ICLR 2026 收录，
        详细记录了多语言数据处理中的独特挑战：
        不同语言需要不同的质量过滤阈值、
        低资源语言的语言识别准确率较低、
        某些书写系统（如阿拉伯文、天城文）
        需要特殊的预处理规则。
        2025--2026 年间没有重大新版本发布；
        与之配套的最新 metadata 层是 propella-annotations，
        propella-1 标注器对 FineWeb-2 全集做了 18 维结构化标注，
        是目前对该数据集最丰富的可用元信息。
        % NEW REF: penedo2025fineweb2 = Penedo, Guilherme and Kydlicek, Hynek and Sabolcec, Vinko and Messmer, Bettina and Foroutan, Negar and Kargaran, Amir Hossein and Raffel, Colin and Jaggi, Martin and Von Werra, Leandro and Wolf, Thomas. FineWeb2: One Pipeline to Scale Them All. arXiv preprint arXiv:2506.20920, 2025 (ICLR 2026).
  \item \textbf{Nemotron-CC}（NVIDIA，arXiv:2412.02595，2024 年 12 月首发，
        2025 年 1 月公开发布；
        2025 年 8 月推出 v2.1 扩展，2025 年 12 月再扩）：
        NVIDIA 构建的\textbf{6.3T token 级}英文预训练数据集
        （4.4T 全局去重的真实 token + 1.9T 合成 token）。
        ...
        （15T token 训练，其中 7.2T 来自 Nemotron-CC）
        在 MMLU 上比 LLaMA 3.1 8B 高 5 分，
        在 ARC-Challenge 上高 3.1 分。
        这种「改写提质」的策略
        为数据质量工程提供了一个新的维度：
        \textbf{不只是过滤掉低质量数据，
        还可以主动改善数据质量}。
        Nemotron-CC v2.1（2025 年末）进一步加入了
        新的代码语料、2.5T 的英文新增 web token
        和专门的 STEM 推理数据，
        与原始 Nemotron-CC 配合使用而非替代。
        % NEW REF: nvidia2024nemotroncc = Su, Dan and Kong, Kezhi and Lin, Ying and Jennings, Joseph and Norick, Brandon and Kliegl, Markus and Patwary, Mostofa and Shoeybi, Mohammad and Catanzaro, Bryan. Nemotron-CC: Transforming Common Crawl into a Refined Long-Horizon Pretraining Dataset. arXiv preprint arXiv:2412.02595, 2024.
\end{itemize}
...
\subsubsection{Common Corpus：完全许可的多语言基线}

法国 \textbf{PleIAs} 团队发布的 \textbf{Common Corpus}~\cite{pleias2026commoncorpus}
是 2026 年完全许可数据路线最重要的工作。
其规模为
\textbf{2,267,302,720,836 token（约 2.27 万亿）}，
由六个商业可用的子集合成，
覆盖 13 种以上的语言，
新增了中文、日语、阿拉伯语、韩语和印地语。
配套的 \textbf{Pleias-1.0} 模型族
包含 0.4B 与 27B 两档，
作为该语料的训练样本。
Common Corpus 已被
\textbf{ICLR 2026 接受为 oral}，
完整的 ICLR 论文（arXiv:2410.22587）
详细记录了数据汇聚、过滤与策展方法学。

\textbf{Common Corpus 2.5}（2026 年 2 月，
为印度 Impact Summit 配套发布）
首次系统性地补充了非西方内容：
新增 270B token，其中 53\% 来自中国、日本、韩国、巴西、印度、
非洲与东南亚等非西方地区。
这次更新让 Common Corpus 从「西方公共领域语料」
正式升级为「全球开放语料」。
PleIAs 同时披露了一个值得深思的统计：
当前主流的 crawl 型数据集（如 FineWeb）
与 Common Corpus 的内容\textbf{重合率不到 2\%}。
也就是说，「公开的可用数据」与「实际被用来预训练的数据」
是两个差异巨大的集合：
开放数据的真正瓶颈不是“被版权遮蔽”，
而是“没有被指引到爬虫的视野范围”。

Common Corpus 在历史脉络中的位置很特殊：
它既不是「在合理使用边缘游走」的网页爬取
（如 The Pile、FineWeb），
也不是「资金雄厚才能负担」的商业许可
（如 OpenAI 的出版商协议），
而是把现有公共领域、开放许可、可商用的所有可识别来源
做了一次系统性的清单化整合。
这个语料的存在使得
「完全合法可商用」不再是一个仅限于 7B--13B 玩具规模的选项：
2.27T token 已经达到
LLaMA 2 (2T) 和 LLaMA 3 8B 早期阶段的训练数据规模，
意味着完全合规的预训练
首次具备了与主流开源模型同台竞争的可行性。
...
\subsubsection{从单一分数到多维标注：propella-1}

2026 年发布的 \textbf{propella-1}~\cite{propella2026}
把质量过滤从「打一个分」推到了「打十八张标签」。
propella-1 是一组 0.6B / 1.7B / 4B 参数的多语言 LLM 标注器，
覆盖 57 种语言，
为每篇文档输出结构化 JSON，
包含\textbf{六大类共 18 个维度}：
核心内容、分类、质量与价值、受众与目的、安全合规、地理相关性。

propella-1 的关键发现是
\textbf{4B 标注器对前沿商业 LLM 的一致率
高于很多更大的通用模型}：
专为标注任务训练的小模型
在标签精度上可以胜过零样本调用的大模型，
而推理成本低一到两个数量级。
这让大规模重标注变得可行。
配套的 \textbf{propella-annotations} 数据集
对 30 亿+ 文档做了完整标注，
覆盖 FineWeb-2、FinePDFs、HPLT 3.0、Nemotron-CC——
这是当前最大、最系统的开放预训练数据 metadata 层。
...
想压低低质 SEO、
想给地理相关性高的文档加权——
都可以在 metadata 层直接做配比策略，
而无需重新跑过滤流水线。
单维度的 perplexity / classifier 分数
将逐渐被这种结构化多维标注取代。

\subsubsection{跨语言质量分类器}

多语言数据的质量过滤一直被视为
比英语更难的开放问题。
EPFL 团队（Yassine Turki、Vinko Sabolčec、Bettina Messmer、Martin Jaggi）
在 2026 年 4 月发表的工作~\cite{aroca2026crosslingual}
首次在 1B 模型 + 103B token 的实验设定上
系统量化了「跨语言池化训练」的收益。

核心结论是：
\textbf{在多语言池上联合训练的质量分类器
一致优于单语 baseline}。
法语在聚合归一化下取得 +1.2pp 的下游提升，
低资源语言则普遍达到或超过其单语 baseline——
这意味着多语言池化对低资源语言尤其友好，
因为它们能从相关高资源语言的标注信号中迁移收益。

但研究者同时强调，
\textbf{单纯扩大数据规模并不稳定}。
多语言池化必须配合两种校正之一才能稳定生效：
\textbf{Q3 采样}（按质量三分位采样以避免高质量段被低质量段稀释）
或 \textbf{retention-rate tuning}
（针对每种语言独立调优保留率）。
缺少这两步时，
跨语言分类器会在中等质量区间出现非单调行为，
反而把一些有用文档过滤掉。
这一发现把「跨语言质量过滤」从一个粗放的工程问题
转变为一个有明确旋钮的可调优过程。
...
Qwen3      & 36T  & --   & 大量 & --   & 大量 & --   \\
LLaMA 4    & 40T  & --   & 大量 & --   & --   & 多模态 \\
DeepSeek-V4-Flash & 32T & --   & 大量 & --   & 大量 & 长上下文 \\
DeepSeek-V4-Pro   & 33T & --   & 大量 & --   & 大量 & 长上下文 \\
\bottomrule
\end{tabular}
\end{center}
...
DeepSeek-V4 系列~\cite{deepseekv4}（2026-04）
则展示了\textbf{为「百万级上下文」量身定做的数据组合}。
V4-Flash 用 32T token 训练，V4-Pro 用 33T token，
两者的数据大量倾向于
长文档（书籍、长篇报告、整库代码）和长链推理样本——
其目的是让模型在 1M token 的原生上下文窗口中
仍然保持稳定的注意力分布与推理能力。
这与早期模型「先训短上下文、再外推」的策略形成对比：
DeepSeek-V4 把长上下文能力\textbf{下沉到了数据层}。
从这个意义上看，
2026 年的数据工程不再只服务「让模型懂语言」，
还在服务「让模型守得住长上下文」——
两个目标对应着完全不同的数据混合策略。
% NEW REF: deepseekv4 = DeepSeek-AI. DeepSeek-V4 Technical Report. arXiv preprint, 2026.
...
\subsubsection{2026 年的三方共识：从「必然崩塌」到「可验证逆转」}

Shumailov 2024 的「必然崩塌」论断
在 2026 年得到了系统性的修正。
两条独立路线在同一时间窗口给出了
\textbf{可验证逆转坍缩}的方法学。
...
\textbf{(a) 验证器路线}。
芝加哥大学的
《Escaping Model Collapse via Synthetic Data Verification》~\cite{uchicago2026verifier}
（arXiv:2510.16657 v2，2026-03-05；
ICLR 2026 poster，2026-04-25）
提出在合成数据回训中
\textbf{注入外部验证器}。
当生成样本必须通过验证器
（执行测试、形式化检查、事实匹配等）
...
\textbf{(b) 新鲜真数据路线}。
arXiv:2602.10531 v2 (2026-02-17)~\cite{kazdam2026collapse}
从统计学角度证明：
只要每一轮训练都注入
\textbf{新鲜的真实数据}，
按 contamination-agnostic 的方式做训练
（即不显式区分真实样本与合成样本），
就足以避免坍缩，
并在极限下恢复真实分布。
这条路线的工程含义最为简洁：
...
把 Shumailov 2024、
UChicago 2026 验证器路线、
以及 arXiv:2602.10531 三方放在一起，
2026 年的共识收敛为三句话：
\textbf{(1) 纯合成数据递归训练会必然坍缩}；
\textbf{(2) 保留真实数据可以下界化测试误差}，
长期不再退化；
\textbf{(3) 验证器只是把坍缩点搬到验证器的知识中心}，
收益与验证器质量同在。
这把模型坍缩从一个
「未来可能的、模糊的风险」
转变为一个
