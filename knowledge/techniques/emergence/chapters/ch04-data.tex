...
\begin{scope}[on background layer]
  % Phase 1: Extraction & filtering (row 1)
  \fill[figBlue!4, rounded corners=4pt]
    (-1.8, -0.85) rectangle (14.7, 1.35);
  % Phase 2: Curation (row 2)
  \fill[figOrange!4, rounded corners=4pt]
    (-1.8, -4.05) rectangle (14.7, -1.85);
\end{scope}

% Phase labels
\node[font=\tiny\itshape, text=figBlue!50, anchor=north west] at (-1.0, 1.3) {Extraction \& Filtering};
\node[font=\tiny\itshape, text=figOrange!50, anchor=north west] at (-1.0, -1.9) {Curation \& Packaging};
...
\end{tikzpicture}
\caption{LLM pretraining data pipeline. Raw Common Crawl HTML passes through
text extraction, language identification, quality filtering (${\sim}60\%$ removed),
multi-level deduplication (${\sim}35\%$ removed), domain classification, ratio-based mixing,
and tokenization. The final high-quality token stream is typically 10--15\% of the original volume.}
