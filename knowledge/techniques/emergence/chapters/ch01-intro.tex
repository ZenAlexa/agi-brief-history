...
\begin{tikzpicture}[
  ...
  stage/.style={
    ...
    draw=figBlue, line width=0.8pt,
    fill=figLightGray,
    minimum width=28mm, minimum height=12mm,
    inner sep=8pt, font=\small\bfseries,
    text=figBlue, align=center
  },
  detail/.style={
    rectangle, rounded corners=2pt,
    draw=figGray!50, line width=0.4pt,
    fill=white,
    minimum width=28mm, minimum height=8mm,
    inner sep=5pt, font=\scriptsize,
    text=figGray, align=center
  },
  arr/.style={
    -{Stealth[length=3mm]}, line width=0.8pt, figBlue
  },
  phasebox/.style={
    rounded corners=6pt,
    draw=none, fill=#1!8,
...
\node[stage] (data) {数据采集};
\node[stage, right=14mm of data] (clean) {清洗与去重};
\node[stage, right=14mm of clean] (tok) {分词};
\node[stage, right=14mm of tok] (pre) {预训练};
\node[stage, below=20mm of data] (sft) {监督微调\\(SFT)};
\node[stage, right=14mm of sft] (rl) {强化学习\\(RLHF/RL)};
\node[stage, right=14mm of rl] (eval) {评估};
\node[stage, right=14mm of eval] (deploy) {部署与推理};

% Detail boxes
\node[detail, below=4mm of data] (d1) {Common Crawl\\Wikipedia\\GitHub};
\node[detail, below=4mm of clean] (d2) {语言过滤\\质量评分\\模糊去重};
\node[detail, below=4mm of tok] (d3) {BPE / Unigram\\词表 32K--262K};
\node[detail, below=4mm of pre] (d4) {next-token pred\\万亿 token\\数千 GPU};
\node[detail, above=4mm of sft] (d5) {对话数据\\指令跟随};
\node[detail, above=4mm of rl] (d6) {奖励模型\\PPO / GRPO};
\node[detail, above=4mm of eval] (d7) {基准测试\\人类评估};
\node[detail, above=4mm of deploy] (d8) {量化 / 蒸馏\\vLLM / TRT-LLM};

% Arrows between main stages
\draw[arr] (data) -- (clean);
\draw[arr] (clean) -- (tok);
\draw[arr] (tok) -- (pre);
\draw[arr] (pre) -- ++(0,-20mm) -| (sft);
\draw[arr] (sft) -- (rl);
\draw[arr] (rl) -- (eval);
\draw[arr] (eval) -- (deploy);

% Phase labels
\node[above=16mm of clean, font=\footnotesize\color{figGreen}] {\textbf{数据工程}};
\node[below=16mm of rl, font=\footnotesize\color{figOrange}] {\textbf{后训练}};
\node[above=16mm of deploy, font=\footnotesize\color{figRed}] {\textbf{部署}};

\end{tikzpicture}
\caption{LLM 训练的完整流水线。上半部分（数据采集 $\to$ 预训练）是数据密集型阶段，下半部分（SFT $\to$ 部署）是精细化阶段。每个阶段的技术挑战和时间成本差异巨大。}
