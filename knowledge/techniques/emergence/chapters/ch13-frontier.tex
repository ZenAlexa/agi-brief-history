...
Boltz-2 由 MIT 团队开发并以 MIT 许可证发布，
...
\textbf{Figure 03}（2026-04-13 发布）~\cite{figure_03_bmw_2026}
设定了人形机器人的新工程基准：
170cm/60kg、50 DoF（手部 22 DoF、力感知 3g 量级）、
搭载 Helix VLA 模型与双 NVIDIA GPU。
约 150 台已部署，
\textbf{BMW Spartanburg 工厂已运行 10 个月并完成 30,000+ 辆 X3 的生产}——
这是首个有公开第三方验证、持续 10 个月以上的人形机器人量产部署。
Figure 03 相比 Figure 01 实现约 90\% 的成本下降，
公司估值 \$39B、累计融资 \$2.6B+，
目标 4 年内量产 10 万台。
...
2026 年 4 月发布的 \textbf{DeepSeek V4}
是首个完整支持华为 Ascend 芯片栈的主流前沿大模型%
~\cite{ascend_deepseek_v4_2026}，
也是首个 \textbf{at-scale} 跑在非 NVIDIA 硬件上的主流通用 LLM。
DeepSeek 在 910C 上实测到的推理性能约为 H100 的 60\%，
但配合 CANN 软件栈与算子级别的优化，
端到端性价比已具备替代 NVIDIA 推理部署的现实可行性。
910C 本身集成 53B 晶体管（SMIC N+2 工艺），
下一代 \textbf{Ascend 920C} 已锁定 Blackwell B200 为对标目标。
