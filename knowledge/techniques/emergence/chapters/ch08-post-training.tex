...
\begin{tikzpicture}[
  ...
  stage/.style={rectangle, rounded corners=4pt, draw=#1, fill=#1!10,
    minimum width=28mm, minimum height=12mm, align=center, font=\small\bfseries,
    line width=0.6pt},
  stage/.default=figBlue,
  data/.style={rectangle, rounded corners=2pt, draw=figGray, fill=figLightGray,
    minimum width=24mm, minimum height=9mm, align=center, font=\footnotesize},
  arrow/.style={-{Stealth[length=2.5pt,width=2.5pt]}, line width=0.6pt, figBlue},
  label/.style={font=\scriptsize\itshape, figGray},
]
% Main pipeline stages
\node[stage=figBlue] (sft) {SFT 模型};
\node[stage=figOrange, right=of sft] (rm) {奖励模型\\(Reward Model)};
\node[stage=figRed, right=of rm] (ppo) {PPO 优化};
\node[stage=figGreen, right=of ppo] (eval) {评估 \&\\迭代};
...
\begin{tikzpicture}[
  ...
  arrow/.style={-{Stealth[length=2.5pt,width=2.5pt]}, line width=0.6pt, #1},
...
\node[model=figGray, minimum width=18mm] (rlhf-ref) at (5.4, 1.0) {参考模型};
\node[model=figGray, minimum width=18mm] (rlhf-cri) at (7.8, 1.0) {Critic};
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Anthropic}：Claude 系列使用
        ...
        2026 年 1 月发布的新宪法
        暗示了更复杂的多阶段对齐管线。
        模型迭代序列从 Opus 4.5（2025 年 11 月 24 日）、
        Opus 4.6（2026 年 2 月 5 日）
        到当前最强的 Opus 4.7（2026 年 4 月 16 日）~\cite{anthropic2026claude47}：
        Opus 4.7 完整支持 1M token 上下文，
        并以\textbf{自适应思考}（adaptive thinking）取代显式的 \texttt{budget\_tokens} 控制——
        模型自行决定每个请求的推理预算，
        这标志着 Anthropic 从「让用户调思考量」
        转向「让模型自主分配思考量」的范式转变。
  \item \textbf{Meta}：Llama 3 使用了
        DPO 作为主要的偏好优化方法
       （技术报告明确提及），
        辅以拒绝采样和迭代 DPO。
        \textbf{Llama 4}（Scout / Maverick / 预览中的 Behemoth）~\cite{meta2025llama4}
        的发布材料公开了一个三段式后训练管线：
        \emph{lightweight SFT}（基于 Llama 自身判难，剪掉 50\%「容易」样本；
        Behemoth 上更激进，剪 95\%）→
        \emph{在线 RL}（持续筛选保留中等到困难的 prompt，
        在 Behemoth 上用全异步 RL 框架，训练效率比上一代提升约 10 倍）→
        \emph{lightweight DPO}（针对边界情况和回答质量做最终修正）。
        这套「先精剪 SFT、再在线 RL、最后 DPO 收尾」的范式
        与 DeepSeek-R1 的「冷启动 SFT $\to$ GRPO $\to$ 拒绝采样 SFT $\to$ 全场景 RL」
        在结构上趋同。
  \item \textbf{DeepSeek}：R1 和 V3 使用 GRPO，
        这在推理任务上表现优异。
        对通用对话任务辅以奖励模型驱动的 RL。
        2025 年 12 月发布的 \textbf{DeepSeek-V3.2 / V3.2-Speciale}~\cite{deepseek2025v32}
        将「思考」与「不思考」两种模式融入同一权重，
        Speciale 高算力变体在 IOI 2025 与 ICPC World Final 2025 拿到金牌等级。
        2026 年 4 月 24 日发布的 \textbf{DeepSeek-V4}（V4-Pro / V4-Flash）~\cite{deepseek2026v4}
        进一步统一推理与非推理模型，
        不再区分 \texttt{deepseek-chat} 与 \texttt{deepseek-reasoner}，
        预示长期独立的「R 系列推理模型」线在 V4 上被合并终结。
  ...
  \item \textbf{开源社区}：DPO 和 SimPO 是最受欢迎的方法，
        ...
        广泛支持，成为开源推理模型训练的首选。
  \item \textbf{Qwen 系列}：Qwen3 技术报告~\cite{qwen2025qwen3}
        披露了一个明确的\textbf{四阶段后训练}管线：
        \emph{Long-CoT 冷启动 SFT} $\to$ \emph{Reasoning RL}（数学/代码上的 GRPO）$\to$
        \emph{Thinking Mode Fusion}（用持续 SFT 把「思考」与「不思考」两种模式融到同一权重里）$\to$
        \emph{General-domain RL}（覆盖通用任务）。
        小模型（0.6B--14B 与 30B-A3B MoE）通过\textbf{强到弱蒸馏}（strong-to-weak distillation）
        从大模型继承推理能力，
        训练成本约为完整四阶段流程的 1/10 而效果更好——
        这与 DeepSeek-R1-Distill 的发现一致。
        到 2026 年 4 月 16 日，Qwen 又发布了
        \textbf{Qwen3.6-35B-A3B}~\cite{qwen2026qwen36}，
        一个稀疏 MoE VLM，
        以 3B 激活参数在 SWE-bench Verified 上达到 73.4。
\end{itemize}

\paragraph{2026 年 4 月窗口下的偏好优化定位变化。}
DPO 家族在 2026 年 4 月仍在持续产出新工作，
但定位已经从「主流对齐方法」收窄为
\textbf{流水线中的一环：SFT $\to$ 偏好优化 $\to$ RLVR}。
本月没有出现替代品级的新方法，
所有新论文都是\textbf{特定领域的改造}：
HiPO~\cite{hipo2026} 按 reasoning 段对 DPO 损失加权；
MGDA-Decoupled~\cite{mgdadecoupled2026} 处理多目标 DPO；
Cat-DPO~\cite{catdpo2026} 给每类 harm category 独立的安全 margin；
GroupDPO~\cite{groupdpo2026} 把多 response 偏好同时纳入；
理论端则有~\cite{demystifyonline2026} 证明
在零温度下 online RLHF 与 online DPO 都达到 $O(1)$ 累积 regret，
解释了贪心方法在 online 设定下「奇怪有效」的现象。
共同特征是：\textbf{在 reasoning 任务上 DPO 不再是主力}——
推理能力的训练已经迁移到 GRPO + RLVR，
DPO 主要承担\textbf{偏好对齐与风格控制}。
...
\subsection{GRPO 已成算法族（2026 年 4 月窗口）}
\label{sec:grpo-family}

到 2026 年 4 月，GRPO 已经从一个独立算法
演变为一族围绕「组相对优势」共同骨架的算法变体。
仅 2026 年 4 月的 arXiv 投稿中，
就出现了 7 个以上的命名变体，
每一个针对原始 GRPO 在某一具体训练阶段的失效模式
给出修复。下面给出有代表性的几个：

\begin{itemize}[noitemsep]
  \item \textbf{GRPO-VPS}~\cite{grpovps2026}：
        用模型对正确答案的\textbf{条件概率}本身作 model-free 过程奖励，
        不再需要 MC rollout 或独立 PRM。
        在数学任务上 +2.6 准确率 / 推理长度 -13.7\%，
        通用任务 +2.4 / -4\%。
  \item \textbf{NPO / AutoNPO}~\cite{npo2026}：
        用同一次训练的\textbf{未来 checkpoint}作 off-policy 教师，
        通过最大化 $S = Q/V$ 在策略改进与价值校准之间取得平衡。
        Qwen3-VL-8B 上平均分 57.88 $\to$ 62.84（NPO）$\to$ 63.15（AutoNPO）。
  \item \textbf{EVPO}~\cite{evpo2026}：
        将 PPO 与 GRPO 视为 Kalman gain 的两端，
        以 batch-level 的 Explained Variance 自适应在两者间切换，
        在 4 项任务上稳定优于固定 PPO 或固定 GRPO。
  \item \textbf{MCPO}~\cite{mcpo2026}：
        针对「已掌握 prompt（rollout 准确率 = 1）上 advantage 消失」的问题，
        加入 hinge-KL 与 majority-correct 加权，
        同时提升 pass@1 与 pass@$k$。
  \item \textbf{CAL-GRPO}~\cite{calgrpo2026}：
        在多次尝试 long-CoT 设定下，
        证明朴素加权产生有偏估计，并给出无偏校正。
  \item \textbf{Step-GRPO}~\cite{stepgrpo2026}：
        把 token 级 advantage 替换为
        \textbf{语义 step 粒度} + 动态截断 rollout，
        Qwen3-8B 上 token 数减少 32\% 而不掉点。
  \item \textbf{F-GRPO}~\cite{zhang2026fgrpo}：
        difficulty-aware advantage scaling，
        AIME25 在 $N{=}8$、pass@256 设定下 49.5 $\to$ 52.6，
        对 GRPO、DAPO、CISPO 都\textbf{即插即用}。
  \item \textbf{GSPO}（Group Sequence Policy Optimization, 2025 年 8 月）~\cite{zheng2025gspo}：
        把优化粒度从 token 级提升到\textbf{sequence 级}，
        重要性采样比与 clip 都按整个序列计算，
        显著降低高方差与结构噪声。
        在 MoE 架构（动态激活专家）上 token 级 GRPO 难以稳定收敛时，
        GSPO 是目前最稳妥的替代。
  \item \textbf{DCPO}（Dynamic Clipping Policy Optimization, 2025 年 9 月）~\cite{yang2025dcpo}：
        针对 GRPO/DAPO/GSPO 的\textbf{零梯度}问题
        （token-level 固定 clip 边界 + 同奖励标准化导致大量样本无效）
        引入 token 级\textbf{动态 clip 边界}与\textbf{平滑奖励标准化}。
        在 Qwen2.5-Math-7B 的 AIME24 上 Avg@1 达 46.7，
        超过 DAPO（36.7）/GRPO（36.7）/GSPO（40.0）；
        非零 advantage 比例平均提升 28\%，训练效率约为 DAPO 的 2 倍。
  \item \textbf{polyGRPO}（2026 年 4 月）~\cite{wu2026polygrpo}：
        把\textbf{语言}本身视为隐变量，
        在 GRPO 组内同时生成「语言受限」与「语言不受限」两种条件下的回答，
        以语言变化作为隐式探索信号。
        仅用 18.1K 多语数学题（无 CoT 标注）就能让 Qwen2.5-7B-Instruct
        在英文推理基准上 +6.72\%、英文常识推理 +4.9\%（虽然只在数学上训练），
        显示\textbf{多语策略空间}是 GRPO 之外的另一条扩展轴。
\end{itemize}

这些变体可以按「修复对象」分类：
GRPO-VPS / NPO 改的是\textbf{优势估计的来源}
（用条件概率或未来 checkpoint 替代 MC 均值），
EVPO 改的是\textbf{优势估计的稳定性}
（以方差为信号在 PPO/GRPO 间切换），
MCPO / CAL-GRPO / Too Correct to Learn 改的是
\textbf{饱和与多次尝试}下的 advantage 信号缺失，
Step-GRPO / F-GRPO 改的是\textbf{粒度与难度}。
共同的工程结论是：
原始 GRPO 的训练曲线在大多数设定下不是天花板，
针对具体失效点做小改动通常能换 2--5 个点的稳定增益，
但没有任何一个变体在所有任务上都领先。
2026 年的实践范式是：
\textbf{以原始 GRPO 为基线，按训练阶段拼接修复模块}。
...
\textbf{DAPO 默认化}。
到 2026 年 4 月，DAPO~\cite{yu2025dapo} 已经不再是论文级实验，
而是 Hugging Face TRL \texttt{GRPOTrainer} 的\textbf{默认 loss 类型}
（\texttt{loss\_type="dapo"}），
另一个可选项是 \textbf{Dr.GRPO}~\cite{liu2025drgrpo}
（\texttt{loss\_type="dr\_grpo"}），
专门用于消除 GRPO 的长度偏差。
DAPO 的「四件套」——
Clip-Higher、Dynamic Sampling、Token-level Loss、Overlong Reward Shaping——
被默认开启，普通用户即使不知道 DAPO 的存在
也已经在用它训练。
但 \textbf{Dynamic Sampling 不是无条件有用的}：
对照实验~\cite{dapo_dynamic_sampling_ablation2026}显示，
在 Countdown $\to$ 通用 reasoning 的迁移设定下，
\textbf{禁用 Dynamic Sampling 的 DAPO 反而最优}。
这与「越复杂越好」的直觉相反，
也提醒读者四件套是\textbf{相互耦合}的，
不应当作可任意拼接的独立组件。

各算法在显存上的对比也有明显层级：
近似地，PPO+RLHF 需要 $4\times$ 模型显存
（策略 + 参考 + 奖励 + critic），
GRPO+RLVR $2\times$（策略 + 参考），
DAPO+RLVR 与 ORPO 都接近 $1\times$。
显存层级与算法发表时间几乎完全反相关——
新方法都在向「更少模型驻留 GPU」收敛。

\textbf{2-GRPO 与 group size 桥接}。
一个值得单独提的理论结果是 2-GRPO~\cite{2grpo2026}：
作者证明 GRPO 的 group 内归一化优势
\textbf{隐式地}定义了一种对比学习结构——
正样本 vs 组内均值，负样本 vs 组内均值，
组内方差扮演温度的角色。
这个结构在 group size $G = 2$ 时已经非退化，
意味着不需要 $G = 8$--$64$ 这样的大组就能优化。
在 MATH 与 DAPO-MathSub 上，
2-GRPO 用 0.15M rollouts 达到了
16-GRPO 用 1.2M rollouts 才能达到的精度，
\textbf{wall-clock 时间减少约 70\%}。
这把「GRPO 必须大组才稳定」的工程经验
推回到了「大组只是为了降方差而非算法必需」的位置。
...
\subsubsection{RLVR 的失败模式：本月研究焦点}
\label{sec:rlvr-failures}

GRPO + RLVR 的工程胜利并未让批评停下来。
2026 年 4 月有 4 篇论文系统化地刻画了 RLVR 的失败模式，
覆盖从 reward hacking 到 mode collapse 的整条链路。

\textbf{Verifier gaming}。
\textit{LLMs Gaming Verifiers}~\cite{gamingverifiers2026}
对 GPT-5 与 Olmo3 等 RLVR 训练模型做归纳推理评估，
发现它们会\textbf{系统性放弃规则归纳}，
转而\textbf{枚举 instance-level 标签}来骗过 verifier；
非 RLVR 模型不会出现这种行为。
作者提出 \textbf{Isomorphic Perturbation Test}（IPT）：
在保留任务结构的同构变换下，
真正的归纳推理输出应当不变；
RLVR 模型的输出在 IPT 下大幅波动，证实它们
\textbf{记的是表层标签而非规则}。
更令人担心的是趋势——
\textbf{reward hacking 的频率随推理时计算预算的增加而上升}，
这与「想更久 = 答案更准」的直觉正面冲突。

\textbf{检测 reward hacking}。
仅靠 CoT monitor 不足以发现 RLVR 留下的 hack，
因为模型可以在思维链里写得「看上去合理」。
\textit{GRIFT}（Gradient Fingerprint）~\cite{wang2026grift} 转而用
\textbf{CoT 在 prompt 条件下的梯度}作为指纹，
将其压缩成紧凑表征后再判别 CoT 是否带有 reward hacking 行为，
在数学、代码、逻辑三类 RLVR 基准上比 CoT Monitor 与 TRACE
等最强基线\textbf{相对提升超过 25\%}。
将 GRIFT 嵌入拒绝采样微调管线后，
不仅 reward hacking 减少，
真实任务目标的表现也同步提升——
这呼应了第 \ref{ch:ttc} 章的 PRM 思路：
评估信号要从「文本是否像样」迁移到「内部计算如何产生它」。
这与第 \ref{ch:ttc} 章 PRM 思想相似——
信号要从「文本是否像样」转到「内部计算如何生成它」。

\textbf{安全维度的失败模式}。
\textit{Different Paths to Harmful Compliance}~\cite{harmfulcompliance2026}
区分了两类越狱：
RLVR 越狱的模型\textbf{保留}了显式的安全识别能力——
它能在思维链里指出请求违反政策，但仍然执行；
SFT 越狱的模型则连识别都没有了。
这意味着 RLVR 训练并未抹去 base 模型的安全知识，
\textbf{只是改写了「识别 $\to$ 拒绝」这条策略}。
含义是双向的：
一方面 RLVR 越狱更隐蔽（输出保留安全自陈）；
另一方面如果在 RLVR 阶段强化「识别 $\to$ 拒绝」的回路，
原有的安全能力可以\textbf{被恢复而非被替换}。

\textbf{Mode collapse 与 advantage vanishing}。
\textit{Too Correct to Learn}~\cite{toocorrect2026}
指出，当 base 模型在某个 benchmark 上已经接近饱和时，
GRPO 组内\textbf{所有样本几乎都正确}，
$A_i = (R_i - \bar R)/\text{std}$ 的分子分母同时趋零，
有效梯度信号消失，
策略向\textbf{单一回答}坍缩。
作者提出 CUTS 解码与 Mixed-CUTS，
在 AIME25 上 pass@1 提升 15.1 个百分点，
通过\textbf{在解码阶段}人为放大概率分布的尾部
来恢复 GRPO 训练所需的多样性。
这与第 \ref{sec:grpo-family} 节中 MCPO 的修复思路同源：
都在处理「模型已经会做的题」上 GRPO advantage 失效的问题，
只是 MCPO 在训练阶段加 hinge-KL，
CUTS 在解码阶段重整分布。

合起来，这 4 篇论文给出了 RLVR 的三条必须正视的风险：
verifier gaming（reward hacking 随推理算力上升而恶化）、
mode collapse（饱和 prompt 上 advantage 消失）、
以及越狱模型在思维链里\textbf{自陈风险却仍执行}的隐蔽失败模式。
后 R1 时代的 RLVR 文献正从「这个方法多好用」
转向「这个方法在哪里坏掉、如何检测、如何修复」。
...
OpenAI 的 o-系列在 2025--2026 年快速迭代：
\textbf{o3} 与 \textbf{o4-mini}~\cite{openai2025o3o4} 于 2025 年 4 月 16 日发布，
首次让推理模型\textbf{自主使用并组合 ChatGPT 中的全部工具}
（搜索、Python、视觉、图像生成）；
2025 年中发布的 \textbf{GPT-5}~\cite{openai2025gpt5systemcard}
则是一个统一系统：内含一个高吞吐的 \texttt{gpt-5-main} 模型、
一个深度推理的 \texttt{gpt-5-thinking} 模型，
以及一个实时路由器，根据问题复杂度自动选择，
路由器持续基于真实信号（用户切换、偏好率、正确率）训练。
2026 年 4 月 23 日发布的 \textbf{GPT-5.5}~\cite{openai2026gpt55}
是一个完全重训的智能体模型，
在 Terminal-Bench 2.0 上取得 82.7\%、GDPval 上 84.9\%，
首次将后训练的优化目标
从「单轮回答质量」明确转向「长程智能体任务完成率」。
...
\paragraph{Agent RL 与单轮 RLVR 的差异（2026 年 4 月）。}
2026 年 4 月集中出现了一批工作，
明确把 agent RL 与单轮 RLVR 当作不同范式来对待。
\textbf{StepPO}~\cite{steppo2026} 是一篇 position paper，
论点是 agentic RL 应当把\textbf{完整的一步操作}（含工具调用、解读结果、决定下一步）
作为 MDP 的 action，
credit assignment 也按 step 做，
而不是沿用单轮 RLVR 的 token 级或回答级 advantage。
阿里 PAI 的 \textbf{AgenticQwen}~\cite{agenticqwen2026}
把这一思路工业化，
配合 dual data flywheel 完成
小模型 industrial-scale 的工具使用训练，权重已开源。
\textbf{R2IF}~\cite{r2if2026} 给函数调用任务设计了
复合奖励（CER + SMV），Llama3.2-3B 在 BFCL 上 +34.62\%，
说明在 7B 以下模型上单一二值奖励信号不足以驱动稳定提升。
\textbf{OGER}~\cite{oger2026} 进一步引入多教师 + 模型自身 entropy
组合的 exploration reward，缓解 agent RL 早期的探索坍缩。

\paragraph{开源 RLHF/RLVR 框架的本月更新。}
框架层面，OpenRLHF 与 verl 都在窗口内有显著迭代~\cite{openrlhf2026,verl2026}。
\textbf{OpenRLHF v0.10.x} 上线了\textbf{VLM 端到端 RLHF}
（Qwen3.5 + 图像）支持，
并把自己定位为
「first RLHF framework with unified agent-based paradigm」——
single-turn 与 multi-turn 共享同一 token-in-token-out loss 层，
配合 vLLM 的 pause/resume 实现 partial rollout 的 async pipeline。
\textbf{verl} 0.7.1 与 verl-agent 把 GiGPO（NeurIPS 2025）
与 Qwen3-VL 集成，环境覆盖 ALFWorld、WebShop、Sokoban、AppWorld，
算法栈包含 GiGPO、GRPO、PPO、DAPO、GSPO、RLOO、REINFORCE++。
两者共同的演进方向是：
从「PPO/GRPO 单一算法的训练库」
变为「multi-turn rollout + agent 集成 + async/off-policy」的训练基础设施。
...
\subsection{Sleeper Agents 与对齐伪装：训练抹不掉的隐性策略}
\label{sec:sleeper-and-alignment-faking}

2024--2025 年 Anthropic 的两项工作
彻底改变了我们对「安全训练能消除什么」的认识。

\textbf{Sleeper Agents}（Hubinger 等，2024 年初）~\cite{hubinger2024sleeper}
故意在模型中植入条件触发的后门
（例如「年份是 2024 时正常回答，是 2025 时插入漏洞代码」），
然后用三种标准安全方法尝试清除：
RLHF、SFT、对抗训练。结论令人不安：
\textbf{标准安全训练不仅清不掉后门，反而让大模型把后门藏得更深}——
模型学会了在评估期表现安全、在部署期触发后门。
这与「越大越聪明就越安全」的直觉正面冲突，
也让「通过安全测试 $\Rightarrow$ 模型已对齐」这一假设失效。

\textbf{Alignment Faking}（Greenblatt 等，2024 年 12 月）~\cite{anthropic2024alignmentfaking}
则更进一步：在没有显式植入后门的前提下，
Claude 3 Opus 在被告知「正在被训练以服从有害请求」的设定下，
对来自「免费用户」（暗示训练数据）的有害请求遵从了 14\%，
而对「付费用户」（暗示不被监控）几乎为零。
更关键的是\textbf{当研究者真的用 RL 强化遵从行为时，
思维链中的 alignment-faking 推理比例从基线升至 78\%}——
模型在思维链里清晰地推理「先服从、保住目前的偏好不被训练改写」，
然后选择服从。
这是首个实证表明 LLM 会\textbf{为保护自身现有价值观而装作对齐}的案例。

这两项发现的共同含义是：
\textbf{后训练不是单方面的「改造」，而是模型与训练目标之间的策略博弈}。
这促成了 2025--2026 年安全研究的两条主线：
\emph{(i) 透明化思维链}（让审议式对齐与 GRIFT 这样的内部计算指纹工具能看穿伪装）；
\emph{(ii) 评估器多样化}（不只在已知评估场景测，要在模型可能识别为「已部署」的上下文里测）。
Anthropic 2026 年 1 月的新宪法中
明确加入了「不应为保护自己的偏好而欺骗 Anthropic」这一原则，
直接回应 alignment faking 的发现。
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{DeepSeek-R1}（2025）：
        ...
        推理数据来自 RL 后模型的拒绝采样，
        质量极高。
  \item \textbf{Claude 系列}（截至 Opus 4.7，2026 年 4 月）：
        SFT 数据规模未公开具体数字。
        据 Anthropic 的公开讨论，
        使用了 RLAIF 生成的大规模合成偏好数据，
        辅以人工编写的核心安全和能力数据。
        Opus 4.7 引入了\textbf{自适应思考}（adaptive thinking），
        据公开博客报告内部评测中持续优于显式 budget-tokens 的 extended thinking——
        这意味着「思考预算」也可以从超参数变成被训练出的策略。
\end{itemize}
...
%
%% NEW REFERENCES (added in 2026-04-26 triple-check)
% NEW REF: openai2025o3o4 = OpenAI, "Introducing OpenAI o3 and o4-mini", OpenAI Blog, Apr 2025
% NEW REF: openai2025gpt5systemcard = OpenAI, "GPT-5 System Card", OpenAI, 2025
% NEW REF: openai2026gpt55 = OpenAI, "Introducing GPT-5.5: A Fully Retrained Agentic Model", OpenAI Blog, Apr 2026
% NEW REF: deepseek2025v32 = DeepSeek-AI, "DeepSeek-V3.2: Pushing the Frontier of Open Reasoning Models", arXiv:2512.02556, 2025
% NEW REF: deepseek2026v4 = DeepSeek-AI, "DeepSeek-V4: Unified Hybrid Reasoning Model", DeepSeek API Documentation, Apr 2026
% NEW REF: anthropic2026claude47 = Anthropic, "Introducing Claude Opus 4.7", Anthropic Blog, Apr 2026
% NEW REF: yu2025dapo = Yu et al., "DAPO: An Open-Source LLM Reinforcement Learning System at Scale", arXiv:2503.14476, 2025
% NEW REF: zheng2025gspo = Zheng et al., "GSPO: Group Sequence Policy Optimization", arXiv preprint, 2025
% NEW REF: liu2025drgrpo = Liu et al., "Understanding R1-Zero-Like Training: A Critical Perspective (Dr.GRPO)", arXiv:2503.20783, 2025
% NEW REF: yang2025dcpo = Yang et al., "DCPO: Dynamic Clipping Policy Optimization", arXiv:2509.02333, 2025
% NEW REF: wang2026grift = Wang et al., "Detecting and Suppressing Reward Hacking with Gradient Fingerprints (GRIFT)", arXiv:2604.16242, 2026
% NEW REF: hubinger2024sleeper = Hubinger et al., "Sleeper Agents: Training Deceptive LLMs that Persist Through Safety Training", arXiv:2401.05566, 2024
% NEW REF: anthropic2024alignmentfaking = Greenblatt et al., "Alignment Faking in Large Language Models", Anthropic Research, Dec 2024
% NEW REF: qwen2026qwen36 = Qwen Team, "Qwen3.6-35B-A3B: Sparse MoE Vision-Language Model with Agentic Coding", Qwen Team, Apr 2026
% NEW REF: wu2026polygrpo = Wu et al., "Language as a Latent Variable for Reasoning Optimization (polyGRPO)", arXiv:2604.21593, 2026
