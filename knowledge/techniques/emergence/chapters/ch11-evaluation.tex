...
\begin{tikzpicture}[
  ...
  root/.style={rectangle, rounded corners=4pt, draw=figBlue, fill=figBlue!15,
    text width=32mm, minimum height=10mm, align=center, font=\small\bfseries},
  l1/.style={rectangle, rounded corners=3pt, draw=figBlue!80, fill=figBlue!8,
    text width=26mm, minimum height=9mm, align=center, font=\small\bfseries,
    inner sep=5pt},
  l2/.style={rectangle, rounded corners=2pt, draw=figGray!60, fill=figLightGray,
    text width=22mm, minimum height=7mm, align=center, font=\scriptsize,
    inner sep=4pt},
  edge from parent/.style={draw=figGray, thick, -stealth},
  level 1/.style={sibling distance=30mm, level distance=18mm},
  level 2/.style={sibling distance=24mm, level distance=16mm},
]

% Root
\node[root] (root) {LLM 基准测试\\全景}
  child {node[l1] (know) {知识}
    child {node[l2] {MMLU\\MMLU-Pro}}
    child {node[l2] {ARC\\GPQA}}
  }
  child {node[l1] (reason) {推理}
    child {node[l2] {GSM8K\\MATH}}
    child {node[l2] {AIME\\FrontierMath}}
  }
  child {node[l1] (code) {代码}
    child {node[l2] {HumanEval\\MBPP}}
    child {node[l2] {SWE-bench\\LiveCodeBench}}
  }
  child {node[l1] (safe) {安全}
    child {node[l2] {TruthfulQA\\BBQ}}
    child {node[l2] {HarmBench\\AdvBench}}
  }
  child {node[l1] (agent) {Agent}
    child {node[l2] {WebArena\\OSWorld}}
    child {node[l2] {BFCL\\ToolBench}}
  };

% Saturated markers (red text annotations)
\node[font=\tiny, text=figRed, anchor=north] at ([yshift=-1mm]know.south west) {饱和};
\node[font=\tiny, text=figOrange, anchor=north] at ([yshift=-1mm]reason.south west) {接近饱和};
\node[font=\tiny, text=figGreen, anchor=north] at ([yshift=-1mm]code.south west) {区分度高};
\node[font=\tiny, text=figGreen, anchor=north] at ([yshift=-1mm]safe.south west) {持续演进};
\node[font=\tiny, text=figBlue, anchor=north] at ([yshift=-1mm]agent.south west) {新兴};
