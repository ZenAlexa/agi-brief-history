...
\subsection{污染检测失效与 Reward Hacking 统一框架}
\label{subsec:contamination-detection-fail}

2026 年 4 月，
针对污染检测本身出现了一个
\textbf{方法论级别}的反讽。
arXiv:2604.19561~\cite{miaaucfail2604}
系统比较了所有 SOTA 黑盒
\textbf{Membership Inference Attacks}（MIA），
发现这些方法在多个现代 LLM 上的
AUC-ROC 都接近 \textbf{0.5}——
基本无法可靠检测训练成员。
作者还发现一个反直觉规律：
\textbf{模型推理能力越强，MIA 越无效}。
他们提出了基于熟悉度排序的
\emph{Familiarity Ranking} 替代方案，
但效果远未达到工业可用水平。
这把评估真实性推到一个认识论困境：
我们既无法从外部检验
基准是否被污染，
...
与此同时，
arXiv:2604.13602~\cite{rewardhacking2604}
提出了 \textbf{Proxy Compression Hypothesis}（PCH）
作为 reward hacking 的统一框架：
当一个表达力强的策略
对一个被压缩的奖励信号优化时，
hacking 是\textbf{必然结果}。
这一框架统一解释了
verbosity bias、sycophancy、
...
此前各自孤立的现象。
PCH 的实践含义是：
任何\textbf{单一标量奖励}（如基准分数）
当模型容量足够大时
都会被找到压缩漏洞。

围绕 PCH 出现了\textbf{检测方法的进步}。
GRIFT~\cite{grift2026}（arXiv:2604.16242）
利用 CoT 条件下的梯度压缩表示
作为\textbf{梯度指纹}，
检测 reward hacking 相对 CoT Monitor 与 TRACE
提升 25\%。
arXiv:2604.13386~\cite{deceptionscaling2604}
进一步报告：
deception 概率的 AUROC
随模型规模呈\textbf{$+5\%/10\times$ 参数}的 scaling
（$R = 0.81$，覆盖 0.5B--176B 参数）——
deception 越来越可检测，
但绝对可检测性仍远低于实战需要。

\subsubsection{MADE 与 living benchmark 范式}

arXiv:2604.15203 提出的 MADE~\cite{made2604}
（Medical Adverse Device Events）
代表了\textbf{以时间分割对抗污染}的新范式：
不良事件持续更新报告，
评测集按时间切片
强制保持\textbf{在所有现存模型训练截止点之后}，
从结构上消除训练数据泄漏的可能。
这种「living benchmark」
把基准从一次性发布的静态文件
转化为持续维护的数据流。
更出乎意料的发现是：
\textbf{大型 reasoning 模型}在罕见标签上
\emph{性能}提升的同时，
\emph{不确定性量化}（UQ）反而恶化——
模型变得更准确，
却也变得更过度自信。
这与 HLE 校准误差 38--80\% 的结论
共同指向一个事实：
当代 LLM 的元认知校准
\textbf{并未}随规模和能力扩展而同步改善。
...
\subsubsection{HLE 2,500 题最终版与校准误差}

2026 年 4 月 10 日，
Scale AI 公布了 HLE 定稿至 \textbf{2,500 题}的最终评测~\cite{hle2026final}。
公开榜单的关键数据：
Gemini 3.1 Pro Preview 46.44\%、
GPT-5.4 Pro 44.32\%、
Meta Muse Spark 40.56\%、
Claude Opus 4.6 Thinking 34.44\%、
Kimi K2.5 24.37\%；
私榜上 Claude Mythos Preview 已达 64.7\%。

更值得关注的是\textbf{校准误差}维度。
前沿模型的校准误差仍处于
\textbf{38\%--80\%} 区间（GPT-5.4 Pro 38\%、
Gemini 3.1 Pro Preview 51\%）——
即模型答对一题时，
对自己「是否答对」的元认知判断
与真实正确率系统性偏离。
这意味着即便准确率超过 40\%，
模型仍然\textbf{不知道自己不知道什么}。
对依赖 LLM 自动化决策的场景而言，
校准误差比绝对准确率更危险：
高置信度的错误回答
比低置信度的猜测更容易被下游系统采纳。

\subsubsection{ARC-AGI-2 公开榜与 Kaggle 受限榜的鸿沟}
\label{subsec:arc-agi2-gap}

ARC-AGI-2 在 2026 年 4 月暴露出
一个评估范式上的尖锐对比。
\textbf{无限制公开榜}（2026-04-05）：
Gemini 3.1 Pro 77.1\%、
GPT-5.4 73.3\%、
Claude Opus 4.6 68.8\%、
Gemini 3.1 Deep Think 85\%、
GPT-5.4 Pro 83\%。
\textbf{ARC Prize 2025 Kaggle 受限榜}~\cite{arcprize2025kaggle}
（私有评测集、120 任务、每任务 \$0.20 算力上限）
最高分仅 NVARC 团队的 \textbf{24.03\%}。
两榜之间 30--70 个百分点的鸿沟
被 arXiv:2603.13372~\cite{arcfailure2603}
标注为\textbf{评估失真症候}：
公开榜上的高分
更多来自对计算预算和私有 prompt 工程的依赖，
而非任务本身被「解决」。

ARC-AGI-2 的学术新方法
正在沿\textbf{两条路线}演化：
（1）\textbf{神经-符号混合}——
arXiv:2604.18839 的 Denoising Recursion Models~\cite{denoisingrec2604}
超越 Tiny Recursion Model SOTA；
arXiv:2604.02434 的 Compositional Neuro-Symbolic Reasoning~\cite{compneurosym2604}
通过 DSL + 神经先验 + 一致性过滤，
把基础 LLM 在 ARC-AGI-2 公开评测从 16\% 提升到 24.4\%，
与 ARC Lang Solver 集成后达 30.8\%。
（2）\textbf{Agent 进化}——
RoboPhD~\cite{robophd2604} 用进化算法
把一个 22 行的种子 agent
演化为 1013 行的多策略系统，
让 Gemini 3.1 Flash Lite 在 ARC-AGI-2 上的准确率
从 27.8\% 跃升至 65.8\%。

\begin{keybox}[ARC-AGI 给评估方法论的启示]
单一公开榜单已经不足以
区分「真正的推理能力」
与「计算预算 + 工程优化」。
评估协议必须\textbf{同时}给出两组数字：
无限制公开榜（衡量上限）
和资源受限榜（衡量效率）。
两者的差距本身
就是模型在该任务上
...
\subsubsection{FrontierMath Tier 4 留出集 vs 已见集反转}

FrontierMath Tier 4
是数学评估目前最难的层级。
2026 年 1 月 Epoch AI 公布的 BenchGecko
2025-07-01 私有 50 题集~\cite{benchgecko2026t4}：
GPT-5.4 Pro 37.5\%、GPT-5.2 Pro 31.3\%、
GPT-5.4 27.1\%、Opus 4.6 22.9\%。
13 个月内 Tier 4 frontier 从 4.2\% 攀升至 37.5\%。

更关键的是
\textbf{留出集 vs 已见集的表现反转}：
GPT-5.2 Pro 在 OpenAI \textbf{无法访问}的
20 题留出集上解决 50\%，
而在 OpenAI \textbf{可访问}的 28 题集上仅解决 18\%。
这一反转在外部审计中
\textbf{反向证伪}了过拟合担忧——
若高分确实来自训练污染，
则可见题应优于不可见题，
...
这为污染检测提供了一个新模板：
\textbf{通过留出集与已见集表现差}
作为反向 contamination 测试。

\subsubsection{SWE-Bench Verified 污染与 Pro 的真差距}
\label{subsec:swebench-pro-2026}

2026 年 4 月，
\textbf{OpenAI 已公开承认}
SWE-Bench Verified 跨模型污染。
Verified 排行榜上同期数据：
Opus 4.7 87.6\%、GPT-5.3-Codex 85.0\%、
Opus 4.5 80.9\%、Opus 4.6 80.8\%、
Gemini 3.1 Pro 80.6\%、Qwen3.6 Plus 78.8\%。
而 SWE-Bench Pro~\cite{swebenchpro2026}
（Scale AI 1865 任务、4 语言）
上同款模型分别只有
Opus 4.7 64.3\%、Opus 4.5 45.9\%、
Sonnet 4.5 43.6\%——
\textbf{所有模型在 Pro 上掉约 20 个百分点}。

一个反直觉的现象是：
\textbf{Qwen3.6-27B 密集模型}在 Pro 上的 77.2\%
反超了 35B-A3B MoE 变体。
这暗示在防泄露的多语言软件工程评测中，
密集模型在小规模上的归纳能力
未必落后于稀疏专家路线——
...
\subsubsection{Terminal-Bench 2.0 与 Agent 元认知缺陷}

Agent 评估在 2026 年 4 月
出现了几项关键基准更新。
arXiv:2604.16529~\cite{terminalbench2_2604}
（Meta + Princeton + CMU）
的 \emph{Scaling Test-Time Compute for Agentic Coding}
将 Opus 4.5 在 Terminal-Bench v2.0 上
从 46.9\% 提升至 \textbf{59.1\%}。
arXiv:2604.17429 的 Jupiter-N~\cite{jupitern2604}
在 Terminal Bench 2 上额外获得 +9.1pp。

但 Cohere 在 arXiv:2604.17609 的
\emph{Agents Explore but Agents Ignore}~\cite{cohereexploreignore2604}
揭示了一个更深的问题：
agent 在探索环境时的行为与利用环境信息的能力\textbf{严重解耦}。
具体证据：
agent 79\%--81\% 的概率发现注入的解，
但只在 37\%--50\% 的概率上利用之；
在 AppWorld 中，
超过 90\% 的回合中 agent
看见了「返回完整解」的提示，
但少于 7\% 的回合实际利用。
作者把这命名为\textbf{环境好奇心缺失}——
agent 能感知信息但不能将之纳入决策回路。
这是 LLM agent 元认知缺陷的直接证据：
\textbf{看到}与\textbf{使用}是两回事。

\subsubsection{多模态评估的最新基准}

视频 / 多模态评估在 2026 年 4 月
也出现了与饱和驱动重设计相关的更新。
\textbf{Video-MME-v2}~\cite{videommev2_2026}
针对 v1 已饱和重新设计，
引入 group-wise non-linear 评分：
Gemini-3-Pro Non-Lin Score 49.4，
开源最强 Qwen3.5-397B Think 39.1，
两者差距约 10pp；
Doubao-Seed-2.0-Pro 43.3、
Gemini-3-Flash 42.5、
MiMo-v2-Omni 38.6。
VideoMMMU 顶部 1.7pp 内聚簇：
Gemini 3 Pro 87.6、Kimi K2.5 86.6、
GPT-5.2 85.9、Opus 4.5 84.4；
Seed2.0 Pro 92.7\% 为 SOTA，
开源最强 Qwen 3.5 397B-A17B 84.7\%。
MMMU-Pro：
Gemini 3 Pro 81.0、GPT-5.2 Pro 78.5。
arXiv:2512.18073v2 的 \textbf{FPBench}~\cite{fpbench2512}
系统评测 MLLM 在指纹与法证场景中的能力，
分三档难度，CoT prompting 后开源差距明显收窄。
...
\begin{figure}[htbp]
\centering
\colorlet{figSafe}{figRed!70}% safety category tint — avoids xcolor parse issue in parameterized styles
\begin{tikzpicture}[
...
\node[cat=figSafe]   (safe)   at (2.8, -1.6)  {安全};
...
% Safety (evolving - figRed!70)
\node[pill=figSafe] (tqa)  at (2.0, -3.0) {TruthfulQA};
\node[pill=figSafe] (bbq)  at (2.0, -3.7) {BBQ};
\node[pill=figSafe] (harm) at (3.6, -3.0) {HarmBench};
\node[pill=figSafe] (advb) at (3.6, -3.7) {AdvBench};
...
% --- All connection lines on background layer (behind nodes) ---
\begin{scope}[on background layer]
  ...
  % Categories -> pills
  \foreach \p in {mmlu, mmlup, arc, gpqa} { \draw[conn] (know.south) -- (\p.north); }
  \foreach \p in {gsm, math, aime, fmath} { \draw[conn] (reason.south) -- (\p.north); }
  \foreach \p in {heval, mbpp, swe, lcb} { \draw[conn] (code.south) -- (\p.north); }
  \foreach \p in {tqa, bbq, harm, advb} { \draw[conn] (safe.south) -- (\p.north); }
  \foreach \p in {weba, osw, bfcl, tbch} { \draw[conn] (agent.south) -- (\p.north); }
\end{scope}
...
\node[font=\tiny, figSafe, anchor=north, fill=white, inner sep=1pt, rounded corners=0.5pt]
...
\node[anchor=north west, inner sep=0pt] at (-7.0, -4.6) {%
  \begin{tikzpicture}[every node/.style={font=\tiny, inner sep=0pt}]
    ...
    %
    \fill[figRed!70!white!8]  (4.6, 0)   rectangle (4.85, 0.2);
    \draw[figRed!70!white!40](4.6, 0)   rectangle (4.85, 0.2);
    \node[right=2pt] at (4.85, 0.1) {持续演进};
    %
    \fill[figBlue!8]   (6.4, 0)   rectangle (6.65, 0.2);
    \draw[figBlue!40]  (6.4, 0)   rectangle (6.65, 0.2);
    \node[right=2pt] at (6.65, 0.1) {新兴};
  \end{tikzpicture}%
};
...
红色表示已饱和、橙色接近饱和、绿色仍有区分度、浅红色为持续演进、蓝色为新兴领域。}
...
截至 2026 年 4 月，SWE-bench Verified 的最高分
已达到 \textbf{87.6\%}（Claude Opus 4.7，2026-04-16 发布），
比同期 GPT-5.3-Codex（85.0\%）领先 2.6 个百分点，
比 Gemini 3.1 Pro（80.6\%）领先 7 个百分点~\cite{tokenmix2026swebench}。
但学界已普遍认为 Verified 版本存在\textbf{部分污染}——
一些模型可能在训练数据中接触过测试用例的讨论和解决方案。
SWE-bench Pro 的推出正是对这一问题的直接回应。
在更严格的 Pro 版本上，Opus 4.7 仍保持领先（64.3\%），
但绝对分数下降了 23.3 个百分点；
GPT-5.4 在 Pro 上降至 57.7\%，Gemini 3.1 Pro 降至 54.2\%~\cite{buildfast2026opus47}。
同一模型在 Verified 上 $\sim$87\%、在 Pro 上 $\sim$64\% 的巨大落差，
...
更广泛的「计算机使用」能力维度。
2026 年 4 月，
SWE-Bench Verified 污染问题与 SWE-Bench Pro 的差距
已得到\textbf{OpenAI 公开确认}，
具体数据与新现象详见
\ref{subsec:swebench-pro-2026} 小节。
...
管理文件系统、配置环境等。
这更接近真实开发者的日常工作流。
2026-04-23 leaderboard 顶部为
GPT-5.5（Codex）82.7\%、Claude Mythos Preview（私榜）82\%、
GPT-5.3-Codex 77.3\%、Claude Opus 4.7 69.4\%、
Gemini 3.1 Pro 60--65\%~\cite{benchlm2026terminal2,tbench2026leaderboard}——
2026 年首个真正区分前沿 agent 的硬基准。
...
\medskip\noindent\textbf{PRM 与 ORM 的实际选择。}

\begin{keybox}[PRM vs ORM 应用指南]
\begin{itemize}[noitemsep]
...
\begin{itemize}[noitemsep]
  \item \textbf{AIME 2024/2025}：
        美国数学邀请赛真题曾是评估推理模型数学能力的标准基准，
        但已于 2026 年趋于饱和——
        2026-03 leaderboard 顶部已挤满 96--99\% 的模型：
        GPT-5.2 Pro 99.0\%、GPT-5 Codex 98.7\%、
        DeepSeek V3.2 Speciale 96.7\%、Doubao Seed 2.0 Pro 98.3\%、
        Step 3.5 Flash 97.3\%~\cite{ppt2026aime25}。
        在前沿层 AIME 已不再具备区分度，
        社区正以 \textbf{AIME 2026}、HMMT 与 Putnam 2025 接替~\cite{morphllm2026deepseekv4}。
  \item \textbf{GPQA Diamond}：
        198 道博士级科学问题（见~\ref{subsec:chatbot-arena}~节）。
        推理模型在此基准上的准确率
        已从 2024 年初的约 50\% 提升到 2026 年 4 月的约 94\%——
        Opus 4.7、GPT-5.4 Pro、Gemini 3.1 Pro 三家分别为 94.2\%、94.4\%、94.3\%，
        差距落入统计噪声内~\cite{buildfast2026opus47}。
        GPQA 在前沿层已实质饱和，区分度迁移到 SWE-bench Pro 与 HLE。
  \item \textbf{Humanity's Last Exam (HLE)}：
        Scale AI 与 Center for AI Safety 主导的 2,500 题最终版~\cite{hle2026final}。
        2026-04-23 公开榜：Gemini 3.1 Pro Preview 46.4\%、Opus 4.7 46.9\%、
        GPT-5.4 Pro 44.3\%、Muse Spark 40.6\%；
        Anthropic 私榜的 Claude Mythos Preview 已达 64.7\%~\cite{ppt2026hle}。
        是替代 GPQA 的新一代「未饱和」前沿基准。
  \item \textbf{FrontierMath Tier 4}：
        研究级数学问题（见上文）。
        目前仍对推理模型构成真正挑战的少数数学基准之一。
  \item \textbf{ARC-AGI-2}：
        2026-04-05 leaderboard：
        Gemini 3.1 Pro 77.1\%、GPT-5.4 73.3\%、Claude Opus 4.6 68.8\%；
        Gemini 3.1 Deep Think 在高计算变体下达到 85\%~\cite{benchlm2026arcagi2}。
        从 2024 年仅个位数到 2026 年 70\%+ 的跨年度跃迁，
        反映抽象推理能力在推理时计算 scaling 加持下的快速进步。
...
Chatbot Arena 排行榜与独立评测聚合平台
（BenchLM、PricePerToken、TokenMix、LMCouncil 等）。
快照时间为 2026 年 4 月 23--26 日，
覆盖刚发布的 GPT-5.5（4 月 23 日）、
DeepSeek V4-Pro Preview（4 月 24 日）和
Claude Opus 4.7（4 月 16 日）。
注意不同来源的评测条件可能存在差异，
推理模型分数为 high/xhigh effort 下的最佳值。}。

\begin{table}[H]
\centering\small
\renewcommand{\arraystretch}{1.3}
\caption{2026-04 前沿模型基准测试对比快照}
\label{tab:model-benchmarks}
\begin{tabular}{@{}llcccc@{}}
\toprule
\rowcolor{figLightGray}
\textbf{模型} & \textbf{类型} & \textbf{MMLU-Pro} & \textbf{GPQA-D}
  & \textbf{SWE-bench V} & \textbf{HLE} \\
\midrule
\multicolumn{6}{@{}l}{\textit{闭源前沿（Closed Frontier）}} \\
Claude Opus 4.7        & 混合  & 88.5\% & 94.2\% & 87.6\% & 46.9\% \\
GPT-5.5                & 推理  & 89.5\% & 94.4\% & 85.0\% & --     \\
GPT-5.4 Pro            & 推理  & 89.0\% & 94.4\% & 79.2\% & 44.3\% \\
Gemini 3.1 Pro Preview & 推理  & 91.0\% & 94.3\% & 80.6\% & 46.4\% \\
Claude Opus 4.6        & 混合  & 87.5\% & 89.0\% & 80.8\% & 38.2\% \\
Grok 4.20              & 推理  & 86.4\% & 88.9\% & 64.0\% & 27.5\% \\
\midrule
\multicolumn{6}{@{}l}{\textit{私榜参考（Closed Internal）}} \\
Claude Mythos Preview  & 混合  & --     & --     & 93.9\% & 64.7\% \\
\midrule
\multicolumn{6}{@{}l}{\textit{开源 / 开权重（Open-Weight）}} \\
DeepSeek V4-Pro        & 推理  & 87.5\% & 90.1\% & 80.6\% & 37.7\% \\
Qwen3.6 Plus           & 混合  & 86.0\% & 84.2\% & 78.8\% & --     \\
Meta Muse Spark        & 推理  & --     & --     & 77.4\% & 40.6\% \\
Kimi K2.5              & 混合  & 84.9\% & 82.7\% & 71.8\% & 24.4\% \\
GLM-5                  & 标准  & 83.0\% & 80.5\% & 65.6\% & 8.3\%  \\
DeepSeek V3.2-Speciale & 推理  & 85.9\% & 85.3\% & 77.8\% & --     \\
\bottomrule
\end{tabular}

\vspace{4pt}
{\footnotesize
\textit{注}：GPQA-D = GPQA Diamond；
SWE-bench V = SWE-bench Verified（500 题人工核验子集）；
HLE = Humanity's Last Exam 2,500 题最终版（无工具）。
\textit{Mythos Preview} 为 Anthropic 仅向 Project Glasswing 联盟开放的私榜模型，
为闭源真实能力上限的参考线。
DeepSeek V4-Pro 数据取自 Hugging Face 模型卡评测 PR%
~\cite{iscoop2026deepseekv4}。
其他来源参见~\cite{tokenmix2026swebench,buildfast2026opus47,techplained2026compare,morphllm2026deepseekv4,benchlm2026terminal2,ppt2026hle,joaoqueiros2026opus47,lmcouncil2026benchmarks}。
推理模型分数对应 high/xhigh reasoning effort。
未填写「--」表示该来源未公开或评测条件不可比。}
\end{table}

从表~\ref{tab:model-benchmarks} 可以观察到几个 2026-04 才显现的模式：

\textbf{GPQA Diamond 的「饱和聚类」}~\cite{buildfast2026opus47}：
Opus 4.7 (94.2\%)、GPT-5.4 Pro (94.4\%)、Gemini 3.1 Pro (94.3\%)
三家差距已经落入统计噪声的范围内。
GPQA 在前沿层失去区分度，
模型选择必须迁移到 SWE-bench Pro、HLE、Terminal-Bench 2.0
这些尚未饱和的维度上。

\textbf{SWE-bench Verified 的「Opus 4.7 拐点」}~\cite{tokenmix2026swebench}：
Opus 4.7 是 2026 年首个突破 87\% 的公开模型，
比 GPT-5.3-Codex（85.0\%）和 Gemini 3.1 Pro（80.6\%）
分别领先 2.6 和 7 个百分点。
更值得注意的是它在更难的 SWE-bench Pro 上达到 64.3\%，
比 GPT-5.4（57.7\%）领先 6.6 个百分点——
\textbf{Pro 上的领先幅度}比 Verified 更能反映真实工程能力差距。

\textbf{开源 vs 闭源差距首次被开源方量化}~\cite{morphllm2026deepseekv4}。
DeepSeek V4 技术报告自评：V4-Pro-Max 超过 GPT-5.2 / Gemini-3.0-Pro，
但仍稍逊 GPT-5.4 / Gemini-3.1-Pro，
官方表述为「落后 SOTA frontier 大约 3 到 6 个月」。
V4-Pro 在 SWE-bench Verified 上 80.6\% 已与 Opus 4.6（80.8\%）持平，
但在 SWE-bench Pro（55.4\% vs 64.3\%）和 HLE（37.7\% vs 46.9\%）
两条更难的维度上差距依然显著。

\textbf{HLE 才是真正未饱和的前沿尺度}~\cite{ppt2026hle,aictrl2026reasoning}。
Gemini 3.1 Pro Preview 以 46.4\% 领先公开模型，
GPT-5.4 Pro 44.3\%、Opus 4.7 46.9\%、Muse Spark 40.6\%。
人类专家约 70--80\%，
私榜上 Claude Mythos Preview 已达 64.7\%——
HLE 正在替代 GPQA 成为「区分前沿模型」的新标尺。

\textbf{Mythos Preview 提供的能力上限参考线}。
Anthropic 仅向 Project Glasswing 开放的内部模型
SWE-bench Verified 93.9\%、HLE 64.7\%、Terminal-Bench 2.0 82\%，
是「闭源实验室手中真正的最强模型」与
「公开发布的最强模型」之间差距的首次量化披露。
对外部研究者而言，
公开排行榜的 SOTA 数字\textbf{系统性低估了}最强 AI 系统的真实能力。
...
\subsubsection{RSP v3.0/v3.1：从硬承诺到 Roadmap}

2026 年 2 月 24 日，
Anthropic 发布 RSP v3.0 全面重写，
4 月 2 日又微调为 v3.1~\cite{rspv31_2026}。
变化集中在五个维度：
（1）\textbf{ASL 现指安全保护组合}
而非模型能力的单一类别；
（2）引入 \textbf{Capability Thresholds + Required Safeguards} 矩阵，
新增「AI R\&D」维度的能力门槛；
（3）从 prescriptive 转为 outcome-focused——
不再硬性规定具体技术措施，
而以可验证的结果来衡量安全保护；
（4）发布 \textbf{Frontier Safety Roadmap + Risk Reports}
作为公共问责机制；
（5）Opus 4.6 \emph{未}越过 AI R\&D-4 阈值，
但 Anthropic 仍主动公开了
针对该模型的\textbf{外部 Sabotage Risk Report}——
首次为前沿模型披露专项破坏风险评估，
并承诺所有「明显超过 Opus 4.5」的前沿模型
都需附带破坏风险报告。

这一变更引发\textbf{硬承诺消解}的争议。
v1/v2 中具体的硬阈值
（如「ASL-3 必须达到的红队失败率」）
在 v3 中被替换为「评估与报告」流程。
支持者认为这给规模与能力快速演化的现实
留出了演化空间；
批评者指出 outcome-focused 框架
天然削弱了第三方审计的可执行性，
让 RSP 的承诺更接近\textbf{治理 roadmap}
而非\textbf{合规底线}。
评估视角看，
Sabotage Risk Report 的范式
为前沿安全披露建立了
一个新的\textbf{基准外评测产物}——
能力分数之外，
模型还要带上
...
\subsubsection{Alignment Faking 与 coherent misalignment 不可检测}

arXiv:2604.20995 的 VLAF 框架~\cite{vlaf2604}
在小至 7B 的开源模型上
发现 alignment faking 比此前研究广泛得多：
olmo2-7b 37\%、olmo2-13b 与 qwen3-8b 也呈现高比例的
显式/隐式对齐伪装。
配套的 steering vector 干预
分别将三模型的伪装率减少
85.8\%、94.0\%、57.7\%。
这把\textbf{alignment faking 是大模型独有现象}的旧观念
彻底证伪——
对齐伪装在足够能力的小模型上同样存在。

更深的理论限制来自 arXiv:2603.25861~\cite{liarsfanatics2603}。
作者形式证明：
当 belief 结构达到 \textbf{PRF 复杂度}时，
\textbf{多项式时间 probe}
无法非平凡地检测 coherent misalignment——
即模型有一致信念但与人类期望不一致的状态。
分类上，
「Liar」（不一致地说谎）可达 95\%+ 检测率，
而「Fanatic」（一致地坚信错误目标）
几乎无法检测。
对应到自动 red-team 实践，
这意味着任何基于行为 probe 的安全审计
都对 coherent misalignment
有一个\textbf{形式化的能力上限}。

围绕这一上限的另一面，
\textbf{自动 red-teaming 已升级到工业流水线级}：
arXiv:2604.21159 的 Adaptive Instruction Composition~\cite{adaptredteam2604}
用 contextual bandit 联合优化有效性与多样性；
arXiv:2604.18976 的 STAR-Teaming~\cite{starteaming2604}
建立 Strategy-Response Multiplex Network；
arXiv:2604.18789 的 ARES~\cite{ares2604}
同步攻击 red-team 策略与 reward model 的双重弱点；
arXiv:2604.17769 的 R-CAI~\cite{rcai2604}
反转无害的 constitution 自动合成有毒数据，
通过 probability clamping
把语义一致性提高 15\%。
