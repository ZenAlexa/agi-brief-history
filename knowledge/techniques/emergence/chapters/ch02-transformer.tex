...
\textbf{2025--2026 年的"可训练稀疏注意力"三件套}
则把焦点从"减少 KV 头数"
转向"减少参与计算的 KV token 数"。
DeepSeek 的 NSA~\cite{yuan2025nsa}
用粗粒度块压缩 + 块级选择 + 滑动窗口三分支，
在 64K 序列上把 forward 加速 9$\times$、
backward 加速 6$\times$。
Moonshot 的 MoBA~\cite{moba2025}
把 MoE 风格的无参 top-$k$ 门控直接搬到注意力上，
让 query 自己挑选 KV 块——
Llama-3.1-8B-1M-MoBA 实测稀疏度达到 95.31\%，
是 Kimi 生产长上下文请求的实际后端。
DeepSeek V3.2 的 DSA~\cite{dsa2025}
则把这条路推到产品级：
FP8 lightning indexer 给历史 token 打分，
fine-grained 选 top-2048 KV，
注意力复杂度从 $O(L^2)$ 降到 $O(Lk)$，
vLLM 在发布日就完成 day-0 集成，
直接拉低了 API 价格 50\% 以上。
NSA、MoBA、DSA 三条路线
共同把"稀疏注意力"从推理后端的工程 trick
升级为预训练时即可学习的一等架构。
...
\textbf{AdaSplash-2}~\cite{adasplash22026}（2026 年 4 月）
在另一条"可微稀疏"路线上取得了关键进展：
让 $\alpha$-entmax 注意力\textbf{首次在 wall-clock 上匹配}
FlashAttention-2。
$\alpha$-entmax 是 softmax 的一个广义化，
通过控制熵参数可以让输出的注意力分布
\textbf{严格稀疏}（精确为零，而非近似为零）：
这使得稀疏度本身成为一个可微参数，
模型可以\textbf{学到}该看哪些 token，
而非靠固定块结构（如 NSA 的窗口）人为切。
长期以来 entmax 因为求解归一化常数 $\tau$
需要迭代，性能远不如 FA2，
被局限在小规模研究。
AdaSplash-2 的核心 trick 是
\textbf{on-the-fly 直方图初始化}——
利用上一步的解作为下一步的 warm start，
把 $\tau$ 的迭代次数压到 1--2 次。
当 block sparsity 超过 60\%
（长上下文场景几乎总是如此）时，
AdaSplash-2 的 per-step 时间
匹配甚至优于 FlashAttention-2，
把"输入相关的稀疏度"从理论玩具
变成了生产级方案。
它与 NSA 形成了清晰对照：
NSA 是固定块结构 + 端到端训练，
AdaSplash-2 是\textbf{学习得到的稀疏模式} + 可微归一化。
...
\textbf{LongRoPE2}~\cite{longrope2}（ICML 2025）
把 RoPE 外推推进到了 "近无损" 阶段。
此前 PI/NTK/YaRN/LongRoPE 在外推到 128K 时
都会损失短上下文性能——
LLaMA3.1 用 YaRN 扩到 128K 后 MMLU 掉 7.56 分，
NTK 掉 4.34，LongRoPE 掉 3.52。
LongRoPE2 的关键观察是：
\textbf{高维 RoPE 分量的训练不充分}是 OOD 问题的根源
（高频频带在原始上下文窗口内本来就转过的圈数少，
扩展上下文后这些维度的相位走向训练时未见过的区间）。
解决方案是用\textbf{演化搜索 + needle-driven perplexity}
寻找新的 rescaling factor，
配合\textbf{混合上下文窗口训练}
让模型同时看到原始 RoPE（短上下文）和 rescaled RoPE（长上下文）。
LLaMA3-8B 用 LongRoPE2 扩到 128K 仅需 \textbf{10B token}
（Meta 官方方案需要 800B token），
保留 \textbf{98.5\%} 的原始短上下文性能，
是当前 RoPE 外推的 SOTA 路径。

\subsubsection{RoPE 与 SSM 的等价性}

2026 年 3 月发布的 Mamba-3~\cite{mamba32026}
给出了一个重要的理论统一：
\textbf{复值 SSM 等价于一种 data-dependent 的 RoPE}。
具体而言，将 SSM 的状态转移矩阵 $\bar A$
推广到复数域，
并把每个频率分量对应的相位
设为依赖于当前输入的函数，
其离散递推的展开形式
就还原为一个对 query/key 的旋转操作——
只是旋转角度不再是 $m\theta_i$ 的固定形式，
而是由输入动态决定的相位序列。

这个等价性在两个方向上都很有启发。
一方面，它解释了 RoPE 为什么有效：
RoPE 实际上是\textbf{固定 $\bar A$ 的复值 SSM 的特例}，
注意力中的位置编码可以看作一个被丢弃了状态变量、
仅保留相位的"退化 SSM"。
另一方面，它指出了 SSM 设计的新方向：
把 RoPE 中精心调过的频率谱
（如 LLaMA 3 的 $\theta_{\mathrm{base}}=500{,}000$、
YaRN 的频率分段）
直接迁移到 SSM 的初始化中，
作为长程衰减的几何先验。

Mamba-3 在 1.5B 参数规模下利用这种统一观点
设计了 exponential-trapezoidal 离散化与 MIMO 更新，
配合"5 层 Mamba-3 + 1 层 NoPE 注意力"的混合栈，
在 H100 上跨 prefill 和 decode 全长序列区间
都比纯 Transformer（vLLM 实现）和 Mamba-2 更快~\cite{mamba3hybridtogether2026}。
SSM 中的 NoPE 注意力层不再需要显式位置编码，
位置信息已经被 SSM 子层隐式编码进了状态轨迹。

\subsubsection{RoPE 的位置鲁棒性}

LLaMA-3-8B、Qwen-3-4B 等使用 RoPE 的开源模型
在 RULER 等长上下文检索基准上有一个共同弱点：
\textbf{对证据 token 的"绝对位置"高度敏感}。
将同一段证据放在 32K 上下文的不同位置，
答案的正确率可以相差几十个百分点。
微软团队在
\textit{Shuffle the Context}~\cite{ropePerturbedSelfDistill2026}
中把这种现象归因于
长上下文微调时模型只见过有限的位置-内容组合，
RoPE 的旋转相位与训练样本的位置分布之间
形成了过强的耦合。

他们的解法是\textbf{RoPE-Perturbed Self-Distillation}：
在训练时对同一段输入构造两个视图——
原始 RoPE 索引与扰动后的 RoPE 索引——
让模型在两种视图下的预测互为蒸馏目标，
强迫输出对位置扰动保持一致。
这只增加少量额外计算，
但在 Llama-3-8B 上 RULER-64K 提升 12.04 个百分点，
Qwen-3-4B 上 RULER-256K 提升 2.71 个百分点，
同时改善了对超出训练长度的外推。
位置鲁棒性因此被列为 PI/YaRN/LongRoPE 之后
RoPE 外推的第四类方法：
不再修改频率，而在训练侧加正则。
...
Flash Attention 4 & B200 & BF16 & 1605--1613 TFLOPS & $\sim$71\% \\
FA4 (FP4 fork) & B200 & NVFP4 & $\sim$1801 TFLOPS & --- \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Flash Attention 4}（2026 年 3 月）~\cite{flashattention4}
正式发布，由 Ted Zadouri、Tri Dao 等人合作完成
（Princeton / Together AI / Meta / Colfax / NVIDIA），
论文同步上线 arXiv:2603.05451。
FA4 不是 FA3 的渐进式调优，
而是\textbf{为 Blackwell 架构（B200/GB200）从零重新设计}的内核：
Hopper 到 Blackwell 的关键变化是\textbf{非对称硬件演进}——
BF16 tensor core 吞吐从 1 PFLOPS 翻倍到 2.25 PFLOPS，
但共享内存带宽和特殊功能单元（SFU，处理 exp 等超越函数）几乎不变。
FA3 在 H100 上利用率约 75\%，但直接搬到 B200
会因为指数运算和共享内存吞吐成为新瓶颈而严重欠优化。
FA4 通过三项创新解决这一不对称扩展问题：

\textbf{（1）异步 MMA 流水线与更大 tile}。
FA4 重新设计了流水线结构，
把矩阵乘法（MMA）操作完全异步化，
同时使用比 FA3 更大的 tile 尺寸来摊薄非矩阵开销。

\textbf{（2）软件模拟的指数与条件 softmax 重缩放}。
SFU 的 exp 吞吐成了瓶颈后，
FA4 用基于多项式的软件 \texttt{exp} 近似把这部分计算搬到 tensor core，
并把 softmax 的最大值跟踪和重缩放
改写成\textbf{条件性}更新——
仅在新块的最大值超过历史最大时才执行 rescale，
减少绝大部分迭代中的非 matmul 操作。

\textbf{（3）Tensor memory 与 2-CTA MMA 模式}。
Blackwell 引入了独立的 tensor memory（独立于共享内存的张量缓存）
和 2-CTA MMA（两个 CTA 协同执行单条 MMA 指令）。
FA4 利用这两项硬件特性减少了反向传播中的共享内存流量
和 atomic add 数量。

实测层面，FA4 在 B200 上 BF16 吞吐达
\textbf{1605--1613 TFLOPS}（约 71\% 的理论峰值利用率），
比 cuDNN 9.13 快 \textbf{1.3$\times$}、
比 Triton 实现快 \textbf{2.7$\times$}（forward 1.1--1.3$\times$、backward 在长序列上一致领先）。
工程层面，FA4 是首个\textbf{完全用 CuTe-DSL（嵌入 Python 的 DSL）}
实现的高性能注意力内核，
编译速度比传统 C++ 模板方案快 20--30$\times$，
极大降低了内核迭代和定制的门槛。
PyTorch 的 \textbf{FlexAttention} 同日宣布已集成 FA4 backend，
覆盖 Hopper 与 Blackwell——
意味着自定义 score modification（ALiBi、document mask、sliding window 等）
也能直接享受 FA4 的速度。

社区在 FA4 之上进一步衍生出了
\textbf{FP4 FA4}（hao-ai-lab 的 \texttt{flash-attention-fp4}）：
利用 Blackwell 的 NVFP4 块缩放 MMA 实现 Q/K/V 与 P 的端到端 FP4，
在 B200 上达到约 \textbf{1801 TFLOPS}（相对 BF16 FA4 再加速 1.39$\times$），
是首个把注意力推到 FP4 量级的工业级实现。
...
\textbf{FA3 的生态延伸}也同样重要。
在 PyTorch 生态中，FA3 已作为 \texttt{third\_party} 子模块
正式集成到主仓库，
通过适配层实现了对 A100（Ampere）和 H100（Hopper）的双重支持，
而 FA4 接过了 Blackwell 这条新前线。
NVIDIA 的 \texttt{cuDNN}（9.13 起）也吸收了 FA4 的多项关键技术，
最新 cuDNN 在 B200 上的注意力性能已与 FA4 持平。

针对 DeepSeek 的 MLA 架构，
社区开发了 \textbf{FlashMLA} 实现~\cite{flashmla2025}：
在 Flash Attention 的 IO-aware 框架上
专门优化了 MLA 的低秩 KV 投影计算路径。
2025 年 4 月版本在 H800 SXM5 上达到 \textbf{660 TFLOPS}，
相对早期版本再提升 5\%--15\%。
2025 年 9 月配合 DeepSeek-V3.2 又推出
\textbf{Sparse FlashMLA}~\cite{flashmla2025}：
为 DSA 的 lightning indexer 生成的 top-$k$ KV 索引
设计了 FP8 稀疏 decoding 内核，
prefill 阶段达到 640 TFLOPS、
decode 阶段达到 410 TFLOPS——
是首个为可训练稀疏 MLA 设计的生产级注意力内核。

更上层的统一抽象由 \textbf{FlashInfer}~\cite{ye2025flashinfer}
（arXiv:2501.01005）提供：
作为 vLLM、SGLang、MLC Engine 等主流 LLM serving 框架的底层
attention/GEMM/MoE kernel 库，
FlashInfer 用 JIT 编译统一了 FA2/FA3、cuDNN、CUTLASS、TensorRT-LLM 多个后端，
v0.4（2025-10）加入 Blackwell 支持，
v0.6（2026-04）已经把 FA4 的 cuTile/CuTeDSL 路径接入主线。
2026 年的事实是：
\textbf{绝大部分推理引擎都不再直接调 FlashAttention，而是通过 FlashInfer 调度}——
FA4 的速度通过这一层透明地传递到上层应用。
...
\subsection{MLA 之后：LCA / DSA / NSMLA / V4 的 CSA+HCA}

MLA 给出了一个清晰的方向——
把注意力计算搬到低维潜空间里去做——
2025--2026 年沿这条路出现了一系列继任者，
它们或者把 MLA 的潜空间与稀疏检索结合，
或者把可训练的稀疏 indexer 加到 MLA 之外，
或者干脆把"压缩"维度推到极致。

\textbf{LCA}（Latent-Condensed Attention）
~\cite{lca2026}
保持 MLA 的低维 KV 不变，
在 MLA 的潜空间内
做一次\textbf{query-aware 的上下文凝聚}：
用聚合分支收集语义内容、
用 anchor 分支保留位置信息，
理论上给出 length-independent 的误差界。
128K 上下文中 prefill 提速 2.5$\times$、
KV cache 再减 90\%，
并且和 GQA 兼容。
LCA 把 MLA 推进的方向是\textbf{进一步向下压}，
而不是横向引入稀疏性。

\textbf{DSA}（DeepSeek Sparse Attention，DeepSeek V3.2）~\cite{dsa2025}
则正好相反：
它保留 MLA，再叠加一层可训练的
\textbf{lightning indexer}——
一个 FP8、少头的轻量打分器，
为每个 query 选 top-2048 KV。
注意力复杂度从 $O(L^2)$ 降到 $O(Lk)$，
vLLM 在发布日（2025-09-29）就完成 day-0 集成，
是首个真正进入生产的可训练稀疏注意力。
工程上的微妙之处是：
DSA 的 indexer 与 MLA 主体使用\textbf{不同的 RoPE layout}，
两者的 paged attention 缓存必须独立维护。

\textbf{NSMLA}（Natively-Sparse MLA）~\cite{nsmla2026}
解决 DSA 的另一半遗留问题——
DSA 的 indexer 是\textbf{推理时}插入的稀疏头，
不能从随机权重直接预训练。
NSMLA 把 indexer 改写成原生形态加入预训练 forward，
TileLang 写训练 kernel，
训练完成后再"折叠"为推理用的 lightning indexer，
这样 vLLM 推理路径完全不变。
它把"trainable sparse attention 第一次产品化"
进一步推到"trainable sparse attention 第一次预训练化"。

到了 \textbf{DeepSeek V4}~\cite{deepseekv4preview2026}
（2026 年 4 月预览：V4-Pro 1.6T/49B、V4-Flash 284B/13B），
注意力被拆成层间交错的三种类型：
\textbf{CSA}（Compressed Sparse Attention）
先沿序列维压缩 KV，
再在压缩后的表示上做 top-$k$ 稀疏选择；
\textbf{HCA}（Heavily Compressed Attention）
做更激进的全局压缩，
近乎只保留语义骨架；
配合滑动窗口层处理局部依赖。
在原生 1M 上下文下，
相对 V3.2 的 MLA+DSA 配置，
V4-Pro 的 per-token 推理 FLOPs 降低 73\%、
KV cache 降低 90\%；
V4-Pro 也成为当前最大的开源权重模型
（>Kimi K2.6 的 1.04T、>GLM-5.1 的 754B），
权重以 MIT 协议放出。
从 MLA 到 LCA、DSA、NSMLA、再到 CSA+HCA 的演进
显示出同一个方向上的两个相互正交的进步：
\textbf{压缩}（潜空间向下走）
与 \textbf{稀疏}（KV token 数横向减少）
正在被同时榨干。
...
用流形约束的超连接替代简单的残差连接，
让深层网络中的信息流动更加高效。
注：DeepSeek V4 系列预览版已于 2026 年 4 月 24 日发布
（V4-Pro 1.6T/49B、V4-Flash 284B/13B），
但官方技术报告未单独披露 mHC 是否被采用、
以及在哪些层位上替换残差连接，
具体应用方式仍有待 V4 完整论文公开后确认。
\end{corebox}
...
\textbf{Samba}（Microsoft，2024）~\cite{samba2024}：
首个把"Mamba + Sliding Window Attention + MLP"
按 \emph{Mamba--MLP--SWA--MLP} 单元重复堆叠的极简 hybrid。
3.8B 模型用 4K 序列预训练 3.2T token 后，
在 1M 上下文上 zero-shot 困惑度仍优于纯 Transformer，
在 4K 微调后能外推到 256K 并在 Passkey Retrieval 上做到 100\% 召回，
解码 128K prompt 比 GQA Transformer 快 3.73$\times$。
Samba 是"linear-time + 精确召回"路线的早期可复现基线。

\textbf{Hymba}（NVIDIA，ICLR 2025 spotlight）~\cite{hymba2025}：
比 Jamba/Samba 更激进的设计——
不再在层间交替 attention 与 SSM，
而是把两者放在\textbf{同一层的并行头}里：
attention 头提供高分辨率精确召回，
Mamba-2 头做高效上下文压缩，两者共享输入。
配合可学习的\textbf{meta tokens}（被预先 prepend 到 prompt，
为 Mamba 初始化隐状态、为 attention 缓解 attention sink），
Hymba-1.5B 超过所有 sub-2B 公开模型，
且超过 Llama-3.2-3B \textbf{1.32 个百分点}的同时
KV cache 减少 11.67$\times$、吞吐提高 3.49$\times$。
Hymba 把"层内并行 hybrid"作为继 Jamba 1:7、Samba 1:1
之后的第三种 hybrid 结构。

\textbf{Zamba2}（Zyphra，2024）~\cite{zamba2024}：
另一种 SSM-attention 极简 hybrid，
特点是\textbf{全模型只用一组共享的 attention 模块}，
通过 LoRA-like 的低秩 projection 在不同层间复用，
进一步把"attention 占比"压到接近 0\%。
2.7B 规模与 Mamba-2、Llama-3.2-3B 在相同训练预算下做了横评。
...
2026 年 3 月，Tri Dao 和 Albert Gu 等人发表了
Mamba-3~\cite{mamba32026}（ICLR 2026 Oral，arXiv:2603.15569）。
Mamba-3 提出了“推理优先”（inference-first）的 SSM 设计哲学，
围绕三项关键改进重新设计了 SSM 的核心计算：

\begin{itemize}[noitemsep]
  \item \textbf{exponential-trapezoidal 离散化}：
        改进梯形数值积分方案，
        把连续时间动力系统更精确地映射到离散递推，
        减少 zero-order hold 等老式离散化的累计误差
  \item \textbf{复值 SSM = data-dependent RoPE}（重大理论统一）：
        当状态转移矩阵 $\bar{A}$ 取值于复数域、
        各特征值相位由当前输入动态生成时，
        SSM 等价于一种数据相关的 RoPE，
        \textbf{把注意力的位置编码与 SSM 的状态演化}统一在同一个数学框架下
  \item \textbf{MIMO 更新}（Multiple-Input Multiple-Output）：
        允许多个输入通道同时更新多个输出通道，
        提升 decoding 的 arithmetic intensity，
        突破 Mamba-1/2 中 SISO 结构的表达瓶颈
\end{itemize}

实测层面，Mamba-3 在 1.5B 规模上
比 Gated DeltaNet 平均提升 0.6 个百分点，
MIMO 再额外贡献 1.2 个百分点（合计 +1.8pp），
parity 任务正确率达到 100\%（Mamba-2 完全失败），
state size 减半仍维持 Mamba-2 的困惑度。
mamba 仓库已合并 \texttt{mamba3.py}。

更值得注意的是它确立的\textbf{混合栈范式}：
Together AI 与论文同期的博文~\cite{mamba3hybridtogether2026} 报告，
"\textbf{每 5 层 Mamba-3 + 1 层 NoPE 自注意力}"
是当前最佳的混合配比——
在 H100、batch=128、1.5B 配置下，
跨 prefill 和 decode 的全 sequence 区间，
该混合栈\textbf{快于 Mamba-2、Gated DeltaNet，
甚至快于 vLLM 上的纯 Transformer}，
且在真实 retrieval 与 needle-in-a-haystack 上
同时优于纯 Transformer 与纯 SSM。
这是"5+1 hybrid"作为继 Jamba 1:7 和稍后 3:1 之后
的又一个独立收敛点（参见 Ch06）。
其“推理优先”的设计理念意味着
架构选择从一开始就面向高效推理场景优化，
而非先追求训练性能再做推理适配。
...
\subsubsection{MiniMax-01：Lightning Attention + MoE 的工业级落地}
\label{sec:minimax01}

MiniMax 在 2025 年 1 月发布的
\textbf{MiniMax-Text-01}~\cite{minimax01}（arXiv:2501.08313）
是\textbf{首个把 linear attention 主导的 hybrid 架构
推到 frontier 商用规模}的模型——
\textbf{4560 亿总参数 / 459 亿活跃参数}的 MoE，
原生支持 1M 训练上下文与 4M 推理上下文，
是发布时世界上上下文最长的开源模型之一。

架构上 MiniMax-01 采用 \textbf{7:1} 的 hybrid 比例：
每 8 层中有 7 层使用 \textbf{Lightning Attention}（一种 IO-aware 的 linear attention 变体），
仅 1 层是标准 softmax 注意力。
Lightning Attention 的核心创新是
\textbf{绕开 cumsum 操作}——
传统线性注意力在因果语言建模中
要计算前缀累积和 \(\sum_{j \leq i}\)，
这一步在 GPU 上是顺序的、效率极差。
Lightning Attention 通过分块 tiling 把累积和拆成
块内并行 + 块间顺序两部分，
块间部分的规模远小于序列长度，
最终保持整体 \(O(n)\) 的同时充分利用 GPU。

MiniMax-01 的工程意义不在某个新算法，
而在于它\textbf{第一次证明 linear attention 主导的架构
能扩展到 \(>\)400B 参数级}：
长期以来 linear attention 被认为只在小模型上有效，
大规模时由于 retrieval 能力不足而崩溃。
MiniMax-01 通过两项关键设计避免了这一陷阱：
（1）保留 1/8 比例的 softmax 注意力层做精确召回；
（2）配套重写了训练/推理基础设施
（包括针对 lightning + softmax + MoE 三种异构层的
计算--通信调度，以及专门的 All-to-all 优化）。
在 4M 推理上下文下 MiniMax-Text-01 接近 GPT-4o 与 Claude-3.5-Sonnet 的水平，
同时上下文窗口比它们大 20--32$\times$。
后续 \textbf{MiniMax M2.5}（2026 年 2 月）把这套架构
进一步压缩到 230B / 10B 活跃，
并在 SWE-bench 上达到 80.2\%（参见 Ch06）。
...
%%
%% --- ADDED 2026-04-26 (monthly update) ---
%% deepseekv4preview2026 = DeepSeek-AI. "DeepSeek V4 Preview: CSA + HCA at 1.6T/49B and 284B/13B." DeepSeek API release notes, April 2026. https://api-docs.deepseek.com/news/news260424
%% mamba32026 = Gu, Albert; Dao, Tri; et al. "Mamba-3: A Reasoning-Inspired State Space Model." ICLR 2026 Oral. arXiv:2603.15569.
%% mamba3hybridtogether2026 = Together AI. "Mamba-3: Beyond the Limits of Transformers." Together AI Blog, 2026-03-17.
%% lca2026 = Anonymous. "Latent-Condensed Attention: Length-Independent Long-Context Inference." arXiv:2604.12452, 2026.
%% ropePerturbedSelfDistill2026 = Microsoft Research. "Shuffle the Context: RoPE-Perturbed Self-Distillation for Long-Context Robustness." arXiv:2604.14339, 2026.
%% adasplash22026 = AdaSplash-2 Authors. "AdaSplash-2: alpha-entmax Sparse Attention Matching FlashAttention-2." arXiv:2604.15180, 2026.
%% moba2025 = Moonshot AI. "MoBA: Mixture of Block Attention for Long-Context LLMs." arXiv:2502.13189, 2025.
%% dsa2025 = DeepSeek-AI. "DeepSeek V3.2-Exp Technical Report (DSA: Lightning Indexer + top-2048 KV)." 2025-09-29.
%% nsmla2026 = Anonymous. "NSMLA: Natively-Sparse MLA from Scratch." OpenReview r9xDZJgIKk, 2026 (under review).
%%
...
%%
%% --- ADDED 2026-04-26 (triple-check pass, agent-A) ---
%% flashattention4 = Zadouri, Ted; Hoehnerbach, Markus; Shah, Jay; Liu, Timmy; Thakkar, Vijay; Dao, Tri. "FlashAttention-4: Algorithm and Kernel Pipelining Co-Design for Asymmetric Hardware Scaling." arXiv:2603.05451, 2026.
%% flashmla2025 = Li, Jiashi et al. "FlashMLA: Efficient Multi-head Latent Attention Kernels (incl. Sparse FlashMLA for DeepSeek V3.2)." GitHub deepseek-ai/FlashMLA, 2025.
%% ye2025flashinfer = Ye, Zihao et al. "FlashInfer: Efficient and Customizable Attention Engine for LLM Inference Serving." arXiv:2501.01005, 2025.
%% longrope2 = Shang, Ning et al. "LongRoPE2: Near-Lossless LLM Context Window Scaling." ICML 2025. arXiv:2502.20082.
%% minimax01 = MiniMax. "MiniMax-01: Scaling Foundation Models with Lightning Attention." arXiv:2501.08313, 2025.
%% hymba2025 = Dong, Xin et al. "Hymba: A Hybrid-head Architecture for Small Language Models." ICLR 2025 spotlight.
%% samba2024 = Ren, Liliang et al. "Samba: Simple Hybrid State Space Models for Efficient Unlimited Context Language Modeling." arXiv:2406.07522, 2024.
%% zamba2024 = Zyphra. "Zamba2: Efficient Hybrid SSM with Shared Attention." 2024.
%% ============================================================
