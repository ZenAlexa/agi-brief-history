...
\begin{tikzpicture}[
  ...
  arr/.style={-{Stealth[length=2.5pt,width=2.5pt]}, line width=0.6pt, figGray!70},
...
\node[lab, above=0.5mm of wq] {Query};
\node[lab, above=0.5mm of wk] {Key};
\node[lab, above=0.5mm of wv] {Value};
\draw[arr] (input.north -| wq) -- (wq);
\draw[arr] (input.north) -- (wk);
\draw[arr] (input.north -| wv) -- (wv);

% Heads
\node[head, above=20mm of wq] (h1) {Head$_1$};
\node[head, above=20mm of wk] (h2) {Head$_2$};
\node[font=\small, text=figGray, above=20mm of $(wk)!0.5!(wv)$] {$\cdots$};
\node[head, above=20mm of wv] (hh) {Head$_h$};

% Head formula (side annotation)
\node[lab, right=6mm of hh, text width=36mm, align=left] {%
  $\displaystyle\text{head}_i = \text{softmax}\!\Bigl(\frac{Q_i K_i^\top}{\sqrt{d_k}}\Bigr)\! V_i$\\[4pt]
  $d_k = d/h$};

% Q,K,V -> Heads (staggered heights to reduce overlap)
\foreach \h/\col/\yoff in {h1/figBlue/3mm, h2/figOrange/6mm, hh/figGray/9mm} {
  \draw[arr, \col!60] (wq.north) -- ++(0,\yoff) -| (\h.south west);
  \draw[arr, \col!60] (wk.north) -- ++(0,\yoff) -| (\h.south);
  \draw[arr, \col!60] (wv.north) -- ++(0,\yoff) -| (\h.south east);
}
...
\begin{tikzpicture}[
  ...
  conn/.style={-{Stealth[length=1.5pt,width=1.5pt]}, line width=0.5pt, figGray!60},
