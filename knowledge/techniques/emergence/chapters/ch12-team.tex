...
\begin{tikzpicture}[
  ...
  region/.style={rectangle, rounded corners=6pt, draw=#1, fill=#1!8,
    minimum width=65mm, minimum height=12mm, inner sep=8pt},
  lab/.style={rectangle, rounded corners=3pt, draw=#1!80, fill=#1!15,
    text width=28mm, minimum height=9mm, align=center, font=\scriptsize,
    inner sep=4pt},
  labsmall/.style={rectangle, rounded corners=2pt, draw=figGray!50, fill=figLightGray,
    text width=22mm, minimum height=7mm, align=center, font=\tiny,
    inner sep=3pt},
  arrow/.style={-stealth, thick, figGray!60},
]

% US region
\node[region=figBlue] (us) at (0, 0) {\textbf{美国}};
\node[lab=figBlue] (oai) at (-3.2, -1.5) {OpenAI\\7,000+人};
\node[lab=figBlue] (ant) at (-1.0, -1.5) {Anthropic\\1,500+人};
\node[lab=figBlue] (gdm) at (1.2, -1.5) {Google\\DeepMind};
\node[lab=figBlue] (meta) at (3.4, -1.5) {Meta FAIR\\1,000+人};
\node[labsmall] (xai) at (-2.1, -3.0) {xAI\\100K H100};
\node[labsmall] (ai2) at (0.1, -3.0) {AI2\\OLMo};
\node[labsmall] (coh) at (2.3, -3.0) {Cohere\\企业级};

% China region
\node[region=figRed] (cn) at (0, -5.0) {\textbf{中国}};
\node[lab=figRed] (ds) at (-3.8, -6.5) {DeepSeek\\150--200人};
\node[lab=figRed] (ms) at (-1.5, -6.5) {月之暗面\\Kimi};
\node[lab=figRed] (zp) at (0.8, -6.5) {智谱AI\\ChatGLM};
\node[lab=figRed] (bd) at (3.1, -6.5) {字节豆包\\Seed};
\node[labsmall] (ow) at (-3.0, -8.0) {01.AI\\Yi 系列};
\node[labsmall] (sf) at (-0.8, -8.0) {阶跃星辰\\视频生成};
\node[labsmall] (mm) at (1.4, -8.0) {MiniMax\\消费产品};
\node[labsmall] (qw) at (3.6, -8.0) {Qwen\\开源};

% EU region
\node[region=figGreen, minimum width=30mm] (eu) at (6.5, -3.0) {\textbf{欧洲}};
\node[lab=figGreen] (mis) at (6.5, -4.5) {Mistral\\600--700人};

% Connecting lines
\draw[arrow] (us.south) -- (oai.north);
\draw[arrow] (us.south) -- (ant.north);
\draw[arrow] (us.south) -- (gdm.north);
\draw[arrow] (us.south) -- (meta.north);
\draw[arrow] (cn.south) -- (ds.north);
\draw[arrow] (cn.south) -- (ms.north);
\draw[arrow] (cn.south) -- (zp.north);
\draw[arrow] (cn.south) -- (bd.north);
\draw[arrow] (eu.south) -- (mis.north);
...
\begin{table}[H]
\centering\small
\caption{主要 AI 组织的 GPU 集群规模估算（2026 年初）}
