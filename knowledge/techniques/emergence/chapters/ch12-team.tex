...
2026 年 3 月 31 日，OpenAI 关闭了
\textbf{史上最大规模的私募融资}：
\$122B 已承诺资本、估值 \$852B，
Amazon、Nvidia、SoftBank 共同领投，
首次开放约 \$3B 额度给散户投资者~\cite{openai_852b_round_2026}。
ARR 已突破 \$25B（39 个月达成，软件公司史上最快），
...
2026 年初连续发布 Claude Opus 4.6 与 \textbf{Claude Opus 4.7}
（2026 年 4 月 16 日 GA）~\cite{anthropic_opus47_release_2026}，
在 SWE-bench、Rakuten-SWE-Bench 与多模态视觉
（图像分辨率提升至 2,576 px、约 3$\times$ 前代）等维度刷新成绩。
更受关注的是
Anthropic 在 Project Glasswing 之下推出的 \textbf{Claude Mythos Preview}
（2026 年 4 月初公开），
被官方描述为「迄今最强的 coding 与 agentic 模型」，
其先进编程能力同时引发了关于网络安全风险的内部讨论~\cite{anthropic_mythos_2026}。
配套发布的 \textbf{Cyber Verification Program}
为合法安全研究者提供高级能力访问通道，
是 Responsible Scaling Policy 在 ASL-3 阶段的具体落地。
产品创新包括：
Claude Code（CLI 级别的自主编程 Agent）、
Computer Use（通过截图和键鼠操作桌面应用）、
以及 Model Context Protocol（MCP，Agent 工具交互的开放标准，
2025 年底捐赠至 Linux 基金会成为行业事实标准）。

Anthropic 的估值演进堪称企业软件史上最陡峭的曲线：
2025 年 3 月以 \$61.5B 完成 \$3.5B 融资，
9 月 Series F 隐含估值 \$183B，
2026 年 2 月 Series G 募集 \$30B、post-money \$380B
（GIC 与 Coatue 共同领投，是有史以来第二大风险投资交易）%
~\cite{anthropic_seriesg_2026}，
4 月中旬 VC 抢入价跃至 \$800B（Amodei 兄妹尚未接受）%
~\cite{anthropic_800b_valuation_2026}。
ARR 增速同样惊人：
2024 年底 \$1B → 2025 年 12 月 \$9B → 2026 年 4 月 \$30B，
是企业软件史上从 \$1B 到 \$30B 用时最短的纪录。
Claude Code 主导开发者工具市场，
企业客户占总收入的 80\%（OpenAI 仅 40\%），
Anthropic 已成为 OpenAI 在 frontier 阵营中最直接的对手。
...
\textbf{Mistral Large 3}（2025 年 12 月发布）~\cite{mistral_large3_2025}
是当前最受关注的开源前沿模型之一：
675B 总参数 / 41B 活跃的细粒度 MoE，
内含 2.5B 视觉编码器，
原生 256K 上下文，使用 3,000 块 H200 从头训练。
更具工程意义的是 Mistral 与 NVIDIA 的协同发布——
\textbf{原生 NVFP4 量化}使 675B 模型可在单个 8$\times$H100/A100 节点完成推理，
在 GB200 NVL72 上实现 5M tokens/sec 每兆瓦的能效。
同期还推出 Ministral-3 小模型家族（3B/8B/14B），
在端侧（DGX Spark、RTX、Jetson）实现 frontier 级能力下沉。
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{DeepSeek-V4}（2026 年 4 月 24 日发布）~\cite{deepseek_v4_release_2026}：
        以\textbf{V4-Pro}（1.6T 总参数 / 49B 活跃参数）
        与 \textbf{V4-Flash}（284B / 13B）双模型形态发布，
        MIT 开源，原生 1M token 上下文，Thinking/Non-Thinking 双模式。
        架构核心创新为 token-wise compression 与 DSA（DeepSeek Sparse Attention），
        KV 缓存仅为 V3.2 的约 10\%、FLOPs 降至 27\%。
        V4-Flash 部分训练运行在华为 \textbf{Ascend 950} 超节点集群上，
        是首个 at-scale 跑在非 NVIDIA 硬件上的主流开源旗舰模型~\cite{deepseek_v4_huawei_2026}。
\end{itemize}
...
是中国首个原生多模态大模型之一，
支持文本、图像、语音的统一理解和生成。
2026 年 4 月 17 日开源的 \textbf{Qwen3.6-35B-A3B}~\cite{qwen36_35b_a3b_2026}
是 Qwen 3.6 代次的首个开放权重模型——
35B 总参数 / 3B 活跃的稀疏 MoE 视觉-语言模型，
原生 262K 上下文（YaRN 可扩展到 1M），
SWE-bench Verified 73.4 分、
Terminal-Bench 2.0 显著超越 Claude Sonnet 4.555，
以约 3B 活跃参数的成本撬动 10$\times$ 体量稠密模型的性能。
该模型采用 Apache 2.0 许可证，
是 Qwen 团队在 \textbf{Agentic Coding} 方向的首张旗舰名片。
...
Meta Superintelligence Labs（MSL）于 2025 年 6 月 30 日由 Zuckerberg 内部宣布成立，
将原 Foundations、产品团队、FAIR 与新设的 frontier 实验室全部并入~\cite{meta_msl_founding_2025}。
组织顶层由前 Scale AI CEO \textbf{Alexandr Wang}（出任 Chief AI Officer）、
前 GitHub CEO \textbf{Nat Friedman} 与
前 Safe Superintelligence CEO \textbf{Daniel Gross} 共同领导，
Wang 的入职附带 Meta 对 Scale AI 的 \$14.3B 战略投资。
MSL 在 2025 年下半年至 2026 年初
以前所未有的现金强度重塑人才市场，
传闻中的部分研究员包裹高达 \$1 亿美元级别。
2026 年 4 月 8 日，MSL 推出首个原生多模态推理模型
\textbf{Muse Spark}（Instant + Thinking 双模式），
计划替换 WhatsApp、Instagram、Facebook 与 Ray-Ban Meta 智能眼镜上的 Llama 聊天后端%
~\cite{wang_msl_muse_spark_2026}。
...
经历了 AI 史上最陡峭的一段曲线。
2026 年 2 月 12 日，Anthropic 完成 \textbf{Series G \$30B 融资}，
由 GIC 与 Coatue 共同领投，post-money 估值 \$380B%
~\cite{anthropic_seriesg_2026}；
4 月中旬 VC 抢入价跃至 \$800B，
...
（2）\textbf{阿布扎比 1GW Stargate UAE（位于 5GW UAE-US AI Campus 内）}。
由 G42 子公司 \textbf{Khazna Data Centers} 主建，
合作方包括 OpenAI、Oracle、Nvidia、Cisco 与 SoftBank。
首期 200MW（约占 1GW Stargate UAE 总容量的 20\%）
预计于 \textbf{2026 年 Q3} 上线，
园区现场超过 5,000 名工人施工、
钢结构重量为埃菲尔铁塔的 1.5 倍~\cite{stargate_uae_5gw_2026}。
完整 Stargate UAE 1GW 园区占地 19.2 km$^2$，
是\textbf{美国本土之外最大的 AI 数据中心项目}，
预计在 3 年内全部建成；
其外延的 5GW UAE-US AI Campus 仍在与其他 hyperscaler 谈判。
同期美国商务部批准向 G42 与沙特 HUMAIN 出口 70,000 张 GB300。
G42 CEO 在达沃斯设定 KPI：
“2026 年生产 10 亿个 AI agent，
每天 12 小时工作消耗约 1GW 电力。”
韩国亦宣布加入 5GW UAE-US AI Campus 计划。
Stargate 自此从“美国本土项目”
