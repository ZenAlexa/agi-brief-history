...
\textbf{4 月 24 日——DeepSeek V4 Preview}（DeepSeek）~\cite{deepseekv4}。
...
更引人注目的是 Anthropic 同期透露了内部模型
\textbf{Claude Mythos Preview}~\cite{anthropicmythos}：
SWE-bench Verified 93.9\%、Terminal-Bench 2.0 82\%、OSWorld-Verified 79.6\%，
仅对 Project Glasswing 联盟成员开放。
Anthropic 评估它「过于强大、暂不公开发布」，
为外界第一次提供了一条真正的 frontier 上限参考线——
公开 GA 与内部最强之间的 gap，约相当于 6\%--10\% 的 SWE-bench Verified 分数。
...
SWE-bench Pro & 64.3\% & Claude Opus 4.7 & 2026-04-16 \\
...
GPQA Diamond & 94.4\% & GPT-5.4 (≈Opus 4.7 94.2 / Gemini 3.1 94.3) & 2026 \\
Aider Polyglot & 89.4\% & Claude Opus 4.5 & -- \\
CursorBench & 61.3 & Composer 2 & 2026-03-19 \\
AAII（综合） & 57 & Opus 4.7 / GPT-5.4 / Gemini 3.1 Pro 三方并列 & 2026-04 \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{4-2} Cursor 3 + Cognition SWE-1.6 预热（4-7 正式发布）；
\textbf{4-7} Claude Mythos Preview 内部上线（93.9\% SWE-bench Verified 数据流出）~\cite{anthropicmythos}；
\textbf{4-8} Meta Muse Spark；
\textbf{4-13} Figure 03 在 BMW Spartanburg 部署满 10 个月、生产 30K+ X3~\cite{figure032026}；
\textbf{4-14} Claude Code 桌面应用重设计 + v2.1.108；
\textbf{4-15}「Agent IDE 哥本哈根时刻」——
OpenAI Agents SDK 升级、Cursor Canvases 与 Windsurf 2.0（含 Devin in Windsurf）同日发布；
...
\textbf{4-23} GPT-5.5（Terminal-Bench 2.0 82.7\%）；
\textbf{4-24} DeepSeek V4 Preview 发布~\cite{deepseekv4}——
Q1 一切「抢跑」的目标终于亮相，
1.6T / 49B 活跃成为最大的开源权重模型，
