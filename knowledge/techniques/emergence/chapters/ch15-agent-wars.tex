...
2025 年 12 月至 2026 年 4 月，
LLM 领域经历了有史以来最密集的技术迭代。
在大约 150 天的时间里，
至少 15 个 frontier 级模型密集发布，
三大 Agent SDK 先后开源，
MCP 从 Anthropic 的内部协议成长为由 Linux 基金会治理的行业标准
（月 SDK 下载 9700 万、>10K servers、八家创始白金会员），
代码 Agent 从辅助工具进化为半自主的工程伙伴，
旗舰模型在 4 月底出现 Claude Opus~4.7、GPT-5.4、Gemini~3.1~Pro
\textbf{三方并列 AAII 57 分}的局面——没有「单一最强」，只有「按任务选模型」。
这不是一个线性渐进的故事，
...
进入 2026 年，AAIF 把治理结构正式落地为
\textbf{Lead Maintainers (BDFL) → Core Maintainers → Maintainers}
三级，规范提案（SEP）改以 PR 形式提交至 \texttt{seps/} 目录~\cite{aaif2026foundation}，
Roadmap 三个方向同步推进：
（1）Governance Maturation——SEP-1302 工作组/兴趣小组制度、
SEP-2085 维护者继任流程；
（2）Protocol Evolution——下一版规范的特性立项；
（3）Adoption——企业部署模式与培训认证。
\textbf{Block Goose} 与 \textbf{OpenAI AGENTS.md}
也明确归入 AAIF 协调范围，
让 MCP 从「Anthropic 的协议」正式升级为「AI 时代的 USB-C」。
...
截至 2026 年 4 月，MCP 生态系统的规模已经令人瞩目~\cite{aaif2026foundation}：
\begin{itemize}[noitemsep]
  \item \textbf{SDK 下载量}：月度 SDK 下载量突破 9,700 万次
        （2025 年 12 月捐赠时数据，截至 2026-03 仍持续增长）；
        Anthropic 官方表态\textbf{每个主流 AI 提供商现在都原生支持 MCP}
...
\textbf{2026-04-08 Agent SDK 三连发}~\cite{anthropicapril8}：
4 月 8 日 Anthropic 一次性发布了三件互相咬合的 Agent 工具，
把 SDK 从「Claude Code 编程包装」升级到「带托管运行时的全栈 agent 平台」：
（1）\textbf{Agent Skills}（\texttt{skills-2025-10-02} beta）——
将「指令、脚本、资源」打包为可被 Claude
\textbf{动态加载}的目录式技能。
首发提供官方 Skills 处理 PowerPoint / Excel / Word / PDF 文件，
并开放 \texttt{/v1/skills} REST API
让开发者上传自定义 Skill。
SDK Python 0.1.49 同步增加 \texttt{skills} 顶级配置项~\cite{claudeagentsdkskills}，
支持 \texttt{["all"]} / 显式列表 / \texttt{None} 三种加载策略。
Skills 的本质是「让 Claude 自己决定何时加载哪个工具说明书」，
是 Anthropic 给「prompt engineering」的官方下沉接口。
（2）\textbf{Claude Managed Agents}（\texttt{managed-agents-2026-04-01} beta）——
\textbf{完全托管}的 agent harness：
Anthropic 替开发者管理 sandbox 容器、内置工具、
SSE streaming 与会话状态，
开发者只需提交 agent 配置和提示。
这是对 Replit Agent / Devin Cloud 模式的官方回应。
（3）\textbf{ant CLI}——
针对 Claude API 的命令行客户端，
原生集成 Claude Code，把
agent / skill / managed-agent 等资源
以 YAML 文件形式版本化管理。
\textbf{Skills + Managed Agents + ant CLI} 三件套
让 Anthropic 第一次提供
「skill 库 → 托管运行时 → 命令行管理面」的完整链路，
对应「写技能 → 跑 agent → 部署运维」三个阶段。
在 ch15 写作时点（2026-04-26），
Skills 仍是 Anthropic 三家 SDK 中
\textbf{唯一}的「skill 即一级原语」实现——
OpenAI Agents SDK 仍以 function calling + handoffs 为核心，
Google ADK 以 graph workflow 为核心。
...
\caption{三大 Agent SDK 架构对比（2026 年 3 月）}\label{tab:agent-sdk-comparison}
...
\textbf{v2.1.108--117 4 月密集迭代}~\cite{claudecodev21x}。
4 月一个月内 Claude Code 又连发约十个版本，
节奏接近\textbf{每 1--2 天一个 release}：
v2.1.108（4/14）引入 \texttt{ENABLE\_PROMPT\_CACHING\_1H} 把 cache TTL 拉长到 1 小时，
新增 \texttt{/recap}，slash command 改为通过 Skill tool \textbf{自动发现}，
\texttt{/undo} 增加 \texttt{/rewind} 别名；
v2.1.110（4/15）实现\textbf{flicker-free} \texttt{/tui fullscreen}，
新增 Push Notification 工具与 scheduled tasks 的 \texttt{--resume}/\texttt{--continue}；
v2.1.111（4/16）随 Opus 4.7 一起上线了介于 high 与 max 之间的
\texttt{xhigh} reasoning effort、\texttt{/effort} 交互滑块、
\texttt{/less-permission-prompts} skill，以及在云端跑多 Agent 代码审查的
\texttt{/ultrareview}——这是\textbf{三 Agent 模式（Planner/Generator/Evaluator）
首次以一级斜杠命令的形式落到产品层}；
同版本 Auto mode 对 Max 订阅者免去 \texttt{--enable-auto-mode} 标志。
v2.1.117（4/22）放弃通过 Glob/Grep 的隐式调用，
改用\textbf{原生 macOS/Linux build 内嵌的 \texttt{bfs} + \texttt{ugrep}}，
并通过 \texttt{CLAUDE\_CODE\_FORK\_SUBAGENT=1} 暴露 fork-style 子代理实验入口。
4 月 14 日同步发布的桌面应用重设计
把 sidebar 多会话、drag-and-drop workspace、
内置 terminal + file editor 与 \textbf{macOS SSH}（此前仅 Linux）
合并到一个原生窗口，把 Claude Code 从「终端工具」推到了「IDE 替代品」的边界。
...
\textbf{Manus 的中美拉锯}~\cite{manusmeta2025}。
通用 agent 这一支由蝴蝶效应（Butterfly Effect）的 \textbf{Manus} 代表。
2025 年 12 月，\textbf{Meta 以 \$2--3B 收购了蝴蝶效应母公司}，
Manus 创始人肖弘改任 Meta VP，
中国市场的 Monica 助手被同步下线。
2025-12-15 发布的 \textbf{Manus 1.6 Max} 与 2026-03-16 发布的
\textbf{Manus Desktop}（macOS / Windows）
通过「My Computer」让 agent 直接操作本地文件，
GAIA 自报领先 Operator / Computer Use / Deep Research。
但 2026-03 中国监管启动对 Manus 高管的出境审查，
让 Meta 的整合时间表至今没有完全落地——
从「中国 agent 之星」到「Meta 子公司」再到「跨境合规风险」，
Manus 是 2026 年中美 AI 产业格局最具象的注脚。
...
提供了渐进式的自主性控制。
2026 年 3 月 19 日发布的 \textbf{Composer 2}~\cite{cursor3composer2}
是 Cursor 自研模型路线的重大升级，
CursorBench 从 Composer 1.5 的 44.2 跃升至 61.3，
Terminal-Bench 2.0 上\textbf{首次有非 Anthropic / 非 OpenAI 模型超越 Opus 4.6}；
Standard 档定价 \$0.50/\$2.50 per MTok（比 Composer 1.5 便宜 86\%），
Fast 档默认 \$1.50/\$7.50。
4 月 2 日发布的 \textbf{Cursor 3} 把多仓库布局、
本地↔云端 agent 切换合并到统一 workspace，
4 月 15 日 \textbf{Canvases} 让 Agent 任务以画布形态并行可视化。

\textbf{Windsurf 2.0 + Devin in Windsurf}（2026-04-15）~\cite{windsurf2devin}。
被 Cognition 收购后的 Windsurf 在 4 月 15 日发布 2.0：
新增 \textbf{Agent Command Center}（Kanban 风格的多 Agent 调度面板）
并把 \textbf{Devin 直接嵌入 Windsurf} 编辑器。
同一天 OpenAI Agents SDK、Cursor Canvases 与 Windsurf 2.0
\textbf{三家同时升级}，社交媒体把这一天称为
「Agent IDE 进入哥本哈根时刻」——
多个独立团队在同一天朝同一方向推进，
是技术范式真正成形的标志。
此前 4 月 7 日 Cognition 还发布了 \textbf{SWE-1.6}~\cite{cognitionswe16}：
接管 SWE-1.5 在 Cerebras 上跑出 950 tok/s，
SWE-bench Pro 较前代提升 10pp，对 Pro 订阅者免费 3 个月。
...
\begin{tabular}{llr}
  ...
  2026 年 4 月 & Claude Opus 4.7 & 87.6\% \\
  2026 年 4 月（内部） & Claude Mythos Preview & 93.9\% \\
  \bottomrule
\end{tabular}
\end{center}
...
（包含 1,865 个多语言问题）
更能反映真实 frontier 差距：
2026 年 4 月 16 日发布的 \textbf{Claude Opus 4.7}
以 \textbf{64.3\%} 占据公开模型第一（较 Opus 4.6 的 53.4\% 暴涨 11pp，
是自 Sonnet 3.5 以来最大单代 coding 跃升），
GPT-5.4 紧随其后 57.7\%、Gemini 3.1 Pro 54.2\%；
更早一代的 Augment Auggie（基于 Opus 4.5）在 Agent Systems 排行榜上
得分 51.8\%。
...
\textbf{4 月 17 日——Qwen3.6-35B-A3B}（阿里巴巴）~\cite{qwen36moe}。
35B 总 / 3B 活跃的稀疏 MoE，Apache 2.0 协议；
是 4 月开源阵营中性价比最优的「办公本可跑」级别模型，
也是 Qwen 系列首次把 MoE 推到 30B 这个端云交界的甜点段。

\textbf{4 月 21 日——Kimi K2.6 GA}（月之暗面）~\cite{moonshotk26}。
继 K2.5 之后仅约 90 天，
K2.6 在 4 月 13 日 Code Preview 进入 beta，
8 天后即 4 月 21 日转 GA——
是 K2 系列\textbf{从 preview 到 GA 最快的一次}。
1T 总参数 / 32B 活跃 / 384 专家的 MoE，
256K 上下文，Modified MIT 协议，
权重发布在 Hugging Face 同时上线 Kimi.com、Kimi App、
官方 API 与 Kimi Code CLI。
关键基准：
\textbf{SWE-bench Pro 58.6\%}（首个公开\textbf{超越 GPT-5.4 (57.7\%)}的开源模型，
也超过 Opus 4.6 的 53.4\% 与 Gemini 3.1 Pro 的 54.2\%；
仍落后 Opus 4.7 的 64.3\%）、
SWE-bench Verified 80.2\%、SWE-bench Multilingual 76.7\%、
Terminal-Bench 2.0 66.7\%、LiveCodeBench v6 89.6\%、
HLE-with-tools 54.0\%（领先 GPT-5.4 的 52.1\%）、
BrowseComp 83.2\%、AIME 2026 96.4\%。
最具说服力的发布演示是
模型在 \textbf{4{,}000 次工具调用、12+ 小时连续运行}下
\textbf{自主用 Zig 重写 Qwen3.5-0.8B 的本地推理路径}并产出可工作的实现，
这是开源模型首次在公开演示中跨过\textbf{半天级别}的 long-horizon coding 阈值。
API 定价 \$0.60 / M input tokens——比 Opus 4.7 的 \$5 便宜约 8 倍——
让 K2.6 成为「在长 horizon agentic 工作流中最具性价比的 frontier 候选」。
配合 K2.5 + Agent Swarm，
Kimi 系列在 4 月底已经在
\textbf{开源旗舰} + \textbf{多 Agent 编排} + \textbf{长 horizon coding}
三个维度同步对齐 frontier 闭源。

\textbf{4 月 24 日——DeepSeek V4 Preview}（DeepSeek）~\cite{deepseekv4,artificialanalysisv4}。
Q1 期间所有「抢跑」的最终目标 V4 终于到来：
\textbf{V4-Pro}（1.6T 总 / 49B 活跃）
\textbf{一举成为最大的开源权重模型}，
超过 Kimi K2.6 1.1T 与 GLM-5 754B；
\textbf{V4-Flash}（284B / 13B）面向高吞吐推理；
原生 1M 上下文，MIT 协议。
架构上以 \textbf{Compressed Sparse Attention（CSA）+
Heavily Compressed Attention（HCA）层间交错}
取代 V3.2 的 DSA，
在 1M 上下文下相对 V3.2 的 per-token FLOPs 仅 27\%、
KV cache 仅 10\%（V4-Flash 更降至 10\% / 7\%）。
V4 还首次完整支持华为 Ascend 芯片栈，
是首个 at-scale 跑在非 NVIDIA 硬件上的主流大模型，
\textbf{DeepSeek V4 + Ascend} 因此构成了
中国第一个端到端模型-芯片主权 stack。
DeepSeek 在 V4 技术报告里给出了精确的自评：
V4-Pro-Max 超过 GPT-5.2 / Gemini-3.0-Pro，
但稍逊于 GPT-5.4 / Gemini-3.1-Pro，
官方原话「落后 SOTA frontier 模型大约 3 到 6 个月」——
首次有开源团队以「月」为单位明确量化与闭源 SOTA 的差距。
公开基准上 V4-Pro-Max 报 \textbf{SWE-bench Verified 80.6\%}、\textbf{LiveCodeBench 93.5}
（后者为 4 月底公开任意模型的最高值）。
API 定价 V4-Pro \$1.74 / \$3.48 per MTok、V4-Flash \$0.14 / \$0.28，
其中 V4-Flash 是\textbf{2026 年首个 input price 跌破 \$0.20 / MTok 的 1M 上下文模型}。
按 Artificial Analysis 排名，V4-Pro 仅\textbf{次于 Kimi K2.6}，
在开源权重 frontier 上的位次为：Kimi K2.6 > DeepSeek V4-Pro > Qwen3.6-Plus > Muse Spark > GLM-5.1。
...
\textbf{GPT-5.4}（2026 年 3 月 5 日）~\cite{openaigpt54}
在推理基准上持续刷新纪录，
进一步巩固了 GPT-5 系列在闭源模型中的领先地位：
1M 上下文、OSWorld-Verified \textbf{75.0\%}（首次越过 72.4\% 的人类基线）、
GDPval 83\%、SWE-bench Pro 57.7\%、
\textbf{BrowseComp 89.3\%}（领先 Opus 4.7 的 79.3\% 达 10pp）、
Terminal-Bench 2.0 75.1\%。
配套的 \textbf{tool search} 大幅减少 prompt 中工具 schema 的 token 占用。
4 月 22 日 OpenAI 发布 \textbf{ChatGPT Workspace Agents}：
Codex 驱动、组织内共享，覆盖 ChatGPT Business / Enterprise / Edu / Teachers 四档，
是 GPTs 的下一代形态——把 agent 产品下沉到企业协同。
4 月 23 日 \textbf{GPT-5.5} 发布，把 Terminal-Bench 2.0 推至 \textbf{82.7\%}；
Codex CLI 同步升级到 0.121.0，Agents SDK 支持 sandbox 内 inspect/run/edit。
GPT-5 的统一架构——
...
\textbf{Claude Opus 4.7}（2026 年 4 月 16 日）~\cite{anthropicopus47}
是 4.6 之后的下一代旗舰，model ID \texttt{claude-opus-4-7}，
同日 GA 上 Anthropic API、Bedrock、Vertex AI 与 Microsoft Foundry。
SWE-bench Verified \textbf{从 80.8\% 跃升至 87.6\%}，
SWE-bench Pro \textbf{从 53.4\% 跃升至 64.3\%}（公开模型第一），
OSWorld-Verified 78\%、Terminal-Bench 2.0 69.4\%、MCP-Atlas 77.3\%。
Anthropic 自己评价这是「自 Sonnet 3.5 以来最大单代 coding 跃升」。
4.7 引入了三个产品级特性：
（1）介于 high 与 max 之间的 \texttt{xhigh} reasoning effort——
让推理时缩放从三档变成四档；
（2）\textbf{task budgets} beta（HTTP header \texttt{task-budgets-2026-03-13}），
开发者可在单一 API 调用中设置「该任务最多花多少 token / 多少美元」，
是 thinking budget 思路从「单次推理」延伸到「整个 agent 任务」的版本；
（3）重构 tokenizer，对相同输入 token 数会比 4.6 多出 0\% 到 35\%——
官方建议在算成本时按 1.0--1.35 的系数估算。
图像分辨率上限也从 4.6 的 1568 px 提升到 \textbf{2576 px / 3.75 MP}。
定价维持 \$5/\$25 per MTok，1M 上下文。

更引人注目的是 Anthropic 在 4 月 7 日同期发布的内部模型
\textbf{Claude Mythos Preview}~\cite{anthropicmythos,anthropicglasswing2026}：
SWE-bench Verified \textbf{93.9\%}、SWE-bench Pro \textbf{77.8\%}（领先 Opus 4.7 公开的 64.3\% 达 13.5pp）、
SWE-bench Multilingual 87.3\%、SWE-bench Multimodal 59.0\%、
Terminal-Bench 2.0 82\%、OSWorld-Verified 79.6\%、CyberGym 83.1\%、HLE-with-tools 64.7\%。
仅对 \textbf{Project Glasswing} 联盟成员开放
（AWS、Apple、Broadcom、Cisco、CrowdStrike、Google、JPMorganChase、
Linux Foundation、Microsoft、NVIDIA、Palo Alto Networks 等十二家创始合作方），
定价 \$25/\$125 per MTok（5 倍于 Opus 4.7 的公开价），
Anthropic 同时承诺 \$100M 模型使用 credits 与 \$4M 直接捐赠给 Apache、
Alpha-Omega/OpenSSF 等开源安全组织。
Mythos Preview 已在内部自主发现 \textbf{thousands of zero-day vulnerabilities}
覆盖主要操作系统和浏览器。
Anthropic 评估它「过于强大、暂不公开发布」，
为外界第一次提供了一条真正的 frontier 上限参考线——
\textbf{公开 GA 与内部最强}的 gap：
SWE-bench Verified 约 6.3pp（87.6 vs 93.9）、
SWE-bench Pro 约 \textbf{13.5pp}（64.3 vs 77.8）、
Terminal-Bench 2.0 约 12.6pp、CyberGym 约 16.5pp。
Pro 与 CyberGym 上的差距远大于 Verified，
表明实验室内部 frontier 与公开 frontier 的真实距离
\textbf{在更难的、更接近真实工程/安全场景的基准上更大}。
...
目前最完整的 Agent 开发技术栈，
4 月一个月内 Claude Code 又连发了约十个版本（v2.1.108--117，详见前节），
\texttt{/ultrareview}（云端多 Agent 代码审查）
是\textbf{三 Agent 模式（Planner/Generator/Evaluator）首次以一级斜杠命令形态落到产品层}。
...
\subsection{2026-04 季度终结的 SOTA 一览}

到 4 月 26 日为止，旗舰对照可以汇成一张表。
最显眼的两个事实：
（1）Claude Opus~4.7、GPT-5.4、Gemini~3.1~Pro 在 AAII 上\textbf{并列 57}~\cite{datalearner2026aaii}，
GPQA Diamond 三方都在 94.2--94.4\% 区间饱和；
（2）Anthropic 内部的 Claude Mythos Preview~\cite{anthropicmythos}
比公开 GA 高出 6--10pp，
是当前可量化的「frontier 真实上限 vs 公开上限」差距。

\begin{table}[H]
\centering\small
\renewcommand{\arraystretch}{1.3}
\caption{2026-04-26 主要 benchmark SOTA}\label{tab:sota-2026-04}
\begin{tabularx}{\linewidth}{l l l l}
\toprule
\rowcolor{figLightGray}
\textbf{Benchmark} & \textbf{SOTA} & \textbf{模型} & \textbf{日期} \\
\midrule
SWE-bench Verified（内部） & 93.9\% & Claude Mythos Preview & 2026-04-07 \\
SWE-bench Verified（公开 GA） & 87.6\% & Claude Opus 4.7 & 2026-04-16 \\
SWE-bench Verified（公开 \#2） & 85.0\% & GPT-5.3-Codex & 2026-03 \\
SWE-bench Pro（内部） & \textbf{77.8\%} & Claude Mythos Preview & 2026-04-07 \\
SWE-bench Pro（公开 GA） & 64.3\% & Claude Opus 4.7 & 2026-04-16 \\
SWE-bench Pro（开源） & 58.6\% & Kimi K2.6 & 2026-04-21 \\
SWE-bench Multilingual（内部） & 87.3\% & Claude Mythos Preview & -- \\
Terminal-Bench 2.0（内部） & 82\% & Claude Mythos Preview & -- \\
Terminal-Bench 2.0（公开） & 82.7\% & GPT-5.5 & 2026-04-23 \\
OSWorld-Verified（内部） & 79.6\% & Claude Mythos Preview & -- \\
OSWorld-Verified（公开） & 78.0\% & Claude Opus 4.7 & 2026-04-16 \\
BrowseComp & 89.3\% & GPT-5.4 & 2026-03-05 \\
MCP-Atlas & 77.3\% & Claude Opus 4.7 & 2026-04-16 \\
CyberGym（内部） & 83.1\% & Claude Mythos Preview & 2026-04-07 \\
HLE-with-tools（开源） & 54.0\% & Kimi K2.6 & 2026-04-21 \\
GPQA Diamond & 94.6\% & Claude Mythos Preview (≈Opus 4.7 94.2 / GPT-5.4 94.4 / Gemini 3.1 94.3) & 2026 \\
Aider Polyglot & 89.4\% & Claude Opus 4.5 & -- \\
LiveCodeBench v6 & 93.5 & DeepSeek V4-Pro-Max & 2026-04-24 \\
GDPval & 84.9\% & GPT-5.5 & 2026-04-23 \\
CursorBench & 61.3 & Composer 2 & 2026-03-19 \\
AAII（综合，闭源） & 57 & Opus 4.7 / GPT-5.4 / Gemini 3.1 Pro 三方并列 & 2026-04 \\
AAII（综合，开源） & -- & Kimi K2.6 \textgreater\ DeepSeek V4-Pro \textgreater\ Qwen3.6-Plus & 2026-04 \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{超越人类基线的 OSWorld}：4 月底，OSWorld-Verified 的人类基线 72.4\%
已被 GPT-5.4（75\%）和 Opus 4.7（78\%）双双越过。
Computer-Use agent 在桌面级别第一次出现「平均强于人类」的旗舰模型，
这是 SWE-bench 80\% 之后第二个真正越界的 agent 基准。
...
\begin{tikzpicture}[
  ...
  layer/.style={
    ...
    minimum width=110mm, minimum height=16mm,
    line width=0.7pt, inner sep=5pt,
    draw=#1!50, fill=#1!12,
  },
  layer/.default=figGray,
  arr/.style={->, >={Stealth[length=2.5pt,width=2.5pt]}, line width=0.6pt, figGray!60},
]

% --- Helper dimensions ---
\def\GAP{3.5mm}  % gap between layers

% Layer 5 (bottom): Memory
\node[layer=figGray] (L5) at (0, 0) {};
\node[font=\small\bfseries, figGray, anchor=west] at ([xshift=5mm]L5.west)
  {记忆层};
\node[font=\scriptsize, figGray!80, anchor=east] at ([xshift=-5mm]L5.east)
  {上下文窗口 · 向量 DB · 执行轨迹};

% Layer 4: Tools
\node[layer=figGreen, above=\GAP of L5] (L4) {};
\node[font=\small\bfseries, figGreen!85, anchor=west] at ([xshift=5mm]L4.west)
  {工具层};
\node[font=\scriptsize, figGreen!70, anchor=east] at ([xshift=-5mm]L4.east)
  {\textbf{MCP} · Function Calling · Code Exec};

% Layer 3: Reasoning
\node[layer=figRed, above=\GAP of L4] (L3) {};
\node[font=\small\bfseries, figRed!85, anchor=west] at ([xshift=5mm]L3.west)
  {推理层};
\node[font=\scriptsize, figRed!70, anchor=east] at ([xshift=-5mm]L3.east)
  {LLM · \textbf{Extended Thinking} · ReAct};

% Layer 2: Orchestration
\node[layer=figOrange, above=\GAP of L3] (L2) {};
\node[font=\small\bfseries, figOrange!85, anchor=west] at ([xshift=5mm]L2.west)
  {编排层};
\node[font=\scriptsize, figOrange!70, anchor=east] at ([xshift=-5mm]L2.east)
  {\textbf{Agent SDK} · Handoffs · 任务分解};

% Layer 1 (top): Human Interface
\node[layer=figBlue, above=\GAP of L2] (L1) {};
\node[font=\small\bfseries, figBlue!85, anchor=west] at ([xshift=5mm]L1.west)
  {人类交互层};
\node[font=\scriptsize, figBlue!70, anchor=east] at ([xshift=-5mm]L1.east)
  {自然语言 · 审批确认 · 反馈};

% --- Inter-layer arrows (downward flow, more visible) ---
\foreach \upper/\lower in {L1/L2, L2/L3, L3/L4, L4/L5} {
  \draw[arr] (\upper.south) -- (\lower.north);
}

% --- Bidirectional feedback arrow on the right (clean) ---
\draw[<->, line width=0.6pt, figGray!45, dashed]
  ([xshift=8mm]L5.east) -- ([xshift=8mm]L1.east);
\path ([xshift=8mm]L5.east) -- ([xshift=8mm]L1.east) coordinate[midway] (fbMid);
\node[font=\tiny, figGray!60, rotate=90, anchor=south]
  at ([xshift=2mm]fbMid) {反馈循环};

% --- Side gradient label (left only, clean) ---
\path (L5.south west) -- (L1.north west) coordinate[midway] (midL);
\node[font=\tiny, figGray!70, rotate=90, anchor=south]
  at ([xshift=-7mm]midL)
...
\section{结语：150 天改变了什么}

从 2025 年 12 月到 2026 年 4 月 26 日，
这 150 天改变了 AI 行业的几个基本假设。

\textbf{2026 年 4 月时间轴补遗}。
3 月之后这一个月的节奏并未减速，反而加快：
\textbf{4-2} Cursor 3 + Cognition SWE-1.6 预热；
\textbf{4-7} Claude Mythos Preview 内部上线（93.9\% SWE-bench Verified、77.8\% Pro 数据流出）~\cite{anthropicmythos,anthropicglasswing2026}
+ Cognition SWE-1.6 GA（Cerebras 950 tok/s、3 个月免费）~\cite{cognitionswe16}；
\textbf{4-8} \textbf{Meta Muse Spark}（HealthBench Hard 42.8、CharXiv 86.4 双 SOTA，AAII 52，
比 Llama 4 Maverick 节省 10 倍以上算力）~\cite{metamuse2026}
\textbf{+}\ Anthropic 三连发：\textbf{Agent Skills}（\texttt{skills-2025-10-02} beta）、
\textbf{Claude Managed Agents}（托管 agent harness）、\textbf{ant CLI}~\cite{anthropicapril8}——
Anthropic 将 SDK 推进为 skill / harness / CLI 三层闭环；
\textbf{4-13} Figure 03 在 BMW Spartanburg 部署满 10 个月、生产 30K+ X3~\cite{figure032026}；
\textbf{4-14} Claude Code 桌面应用重设计 + v2.1.108~\cite{claudecodev21x}（\texttt{ENABLE\_PROMPT\_CACHING\_1H}、\texttt{/recap}）；
\textbf{4-15}「Agent IDE 哥本哈根时刻」——
OpenAI Agents SDK 升级、Cursor Canvases 与 Windsurf 2.0（含 Devin in Windsurf）同日发布~\cite{windsurf2devin}；
\textbf{4-16} Claude Opus 4.7~\cite{anthropicopus47} + Claude Code v2.1.111（\texttt{/ultrareview}、\texttt{xhigh}）；
\textbf{4-17} Qwen3.6-35B-A3B 开源~\cite{qwen36moe}；
\textbf{4-21} \textbf{Kimi K2.6 GA}~\cite{moonshotk26}——
首个公开开源模型超越 GPT-5.4 SWE-bench Pro（58.6\% vs 57.7\%），
4{,}000 工具调用 / 12 小时长跑演示，是开源模型首次跨过半天级别 long-horizon 阈值；
\textbf{4-22} Claude Code v2.1.117 + ChatGPT Workspace Agents~\cite{openaigpt54}；
\textbf{4-23} GPT-5.5（Terminal-Bench 2.0 82.7\%、GDPval 84.9\%、SWE-bench Pro 58.6\%）；
\textbf{4-24} DeepSeek V4 Preview 发布~\cite{deepseekv4,artificialanalysisv4}——
Q1 一切「抢跑」的目标终于亮相，
1.6T / 49B 活跃在开源权重 frontier 上仅\textbf{次于 Kimi K2.6}（发布仅 3 天前），
官方自评落后 frontier 闭源 3--6 个月。
四月最后一周（4-21 K2.6 → 4-23 GPT-5.5 → 4-24 V4）
「\textbf{每两天一个旗舰}」的节奏
是 LLM 历史上从未出现过的发布密度。

\textbf{Agent 不再是未来技术}。
Claude Code 的 \textbf{118,000+} GitHub stars（4 月底数据）、
每周百万级的 Codex CLI 用户、
SWE-bench Verified \textbf{87.6\% 公开 / 93.9\% 内部}的解决率
（Claude Opus 4.7 / Mythos Preview），
SWE-bench Pro 内部已达 \textbf{77.8\%}（Mythos Preview）——
比公开 frontier 高出 13.5pp，
这些数字表明 Agent 已经从实验室走向了
开发者的日常工作流，
而\textbf{实验室内部 frontier 与公开 frontier 之间的距离}
在更难的真实工程基准上比观察者过去估计的更大。
...
这 150 天只是开始。
DeepSeek V4 已于 4 月 24 日如期发布，
MCP Dev Summit 已在 4 月 2--3 日于纽约举行，
NVIDIA Vera Rubin 将在年内量产，
旗舰阵营出现 AAII 三方并列 57 的局面，
竞争的烈度只会继续加剧。
...
可靠性差距、安全挑战和信任建设
都是需要数年工程积累才能克服的障碍。
但 2025 年 12 月到 2026 年 4 月这 150 天的进展
已经证明了一件事：
\textbf{这个方向是正确的}。
