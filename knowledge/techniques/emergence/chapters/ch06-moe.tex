...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=3pt, minimum height=10mm,
              minimum width=22mm, inner sep=6pt, align=center},
  expert/.style={box, fill=figBlue!12, draw=figBlue!50},
  router/.style={box, fill=figOrange!15, draw=figOrange!60},
  shared/.style={box, fill=figGreen!12, draw=figGreen!50},
  combine/.style={box, fill=figRed!10, draw=figRed!50},
  input/.style={box, fill=figLightGray, draw=figGray},
  arrow/.style={-{Stealth[length=2.5pt,width=2.5pt]}, line width=0.6pt, figGray!60},
  dasharrow/.style={-{Stealth[length=2.5pt,width=2.5pt]}, line width=0.6pt, dashed, figGray!50},
]

% Input
\node[input] (input) {Token $\mathbf{x}$\\{\scriptsize 隐状态 $d$-维}};

% Router
\node[router, above=10mm of input] (router) {Router\\$W_r \mathbf{x} \to \mathbb{R}^N$};

% Top-K selection
\node[box, fill=figOrange!8, draw=figOrange!40,
      above=8mm of router, minimum width=32mm] (topk)
      {Top-K + Softmax\\{\scriptsize $g_1, g_2, \ldots, g_K$}};

% Experts in a compact row
\node[expert, above=14mm of topk, xshift=-24mm] (e1) {Expert 1};
\node[expert, above=14mm of topk] (e2) {Expert $i$};
\node[expert, above=14mm of topk, xshift=24mm] (eN) {Expert $N$};
\node[font=\small, text=figGray] at ($(e2.east)!0.5!(eN.west)$) {$\cdots$};

% Shared expert (same level as experts, further right)
\node[shared, above=14mm of topk, xshift=54mm] (shared) {Shared\\Expert};

% Combine (wide enough to span all experts + shared)
\node[combine, above=14mm of e2, xshift=15mm, minimum width=88mm] (combine)
     {加权求和\quad $\mathbf{y} = \sum g_i \cdot E_i(\mathbf{x}) + E_s(\mathbf{x})$};

% Output
\node[input, above=8mm of combine] (output)
     {Output $\mathbf{y}$\quad{\scriptsize (残差连接后)}};

% --- Arrows: main flow ---
\draw[arrow] (input) -- (router);
\draw[arrow] (router) -- (topk);

% Arrows: topk -> experts (clean fan-out)
\draw[arrow, figBlue] (topk.north) -- ++(0, 3.5mm) -| (e1.south)
...
\draw[arrow, figBlue] (topk.north) -- ++(0, 3.5mm) -| (eN.south)
     node[pos=0.75, right, font=\scriptsize, figBlue] {$g_K$};

% Arrows: experts -> combine (all straight up into combine bottom)
\draw[arrow, figBlue] (e1.north) -- (e1.north |- combine.south);
\draw[arrow, figBlue] (e2.north) -- (e2.north |- combine.south);
\draw[arrow, figBlue] (eN.north) -- (eN.north |- combine.south);

% Arrow: shared -> combine (straight up)
\draw[arrow, figGreen] (shared.north) -- (shared.north |- combine.south);

% Arrow: combine -> output
\draw[arrow] (combine) -- (output);

% Bypass: input -> shared (clean right-side path, no overlap)
\draw[arrow, figGreen, rounded corners=3pt]
  (input.east) -- ++(42mm, 0) |- (shared.south)
  node[pos=0.12, above, font=\scriptsize, figGreen] {全部 token};

% Router annotation (LEFT side, away from bypass arrow)
\node[left=4mm of router, font=\scriptsize, text=figGray, text width=22mm, align=right]
     {线性投影\\无偏置、无激活\\参数 ${\sim}\, d \!\times\! N$};

% Inactive experts annotation (LEFT side)
\draw[dasharrow] (topk.west) -- ++(-12mm, 0)
     node[left, font=\scriptsize, text=figGray!70, text width=20mm, align=right]
     {未选中 $N{-}K$ 个\\专家零 FLOP};
...
Mistral 在 Mixtral 8x22B 之后\textbf{长达 20 个月没有发布新的 MoE 模型}，
直到 2025 年 12 月才以 \textbf{Mistral Large 3}
（675B / 41B，细粒度 MoE）回归——
具体细节见后文 \ref{sec:mistral-large3}。
这一缺席期间，"Mixtral 8 个大专家 + Top-2"的配方
被 DeepSeek 的"256 个细粒度小专家 + Top-8"
和 Qwen3 的 "128 个专家 + Top-8"
逐步证明在 frontier 规模下并非最优；
Large 3 的回归直接采用了细粒度配置，
也意味着 Mixtral 风格的"少而大专家"路线
在 frontier 旗舰中基本退场。
...
\subsection{DeepSeek V4 Preview：CSA + HCA 双稀疏 MoE}
\label{sec:deepseek-v4-moe}

2026 年 4 月 24 日，DeepSeek 公布
\textbf{V4 系列预览版}~\cite{deepseekv4preview2026}：
\textbf{V4-Pro}（1.6T 总参数 / 49B 活跃）
与 \textbf{V4-Flash}（284B / 13B），
原生支持 1M 上下文，
权重以 MIT 协议开源。
V4-Pro 一举成为当前最大的开源权重模型，
超过 Kimi K2.6 的 1.04T、GLM-5.1 的 754B。

V4 在注意力侧的核心改动是把 V3.2 的 MLA + DSA
进一步拆成层间交错的两种压缩注意力（已在 Ch02 详述）：
\textbf{CSA}（Compressed Sparse Attention）
先沿序列维压缩 KV、再做 top-$k$ 稀疏选择；
\textbf{HCA}（Heavily Compressed Attention）
做更激进的全局压缩；
配合滑动窗口处理局部依赖。
在 1M 上下文下，
相对 V3.2，V4-Pro 的 per-token 推理 FLOPs 降低 \textbf{73\%}、
KV cache 降低 \textbf{90\%}；
V4-Flash 的两个数字分别是 90\% 与 93\%。
MoE 主体延续 V3 的细粒度专家 + 共享专家路线，
后训练采用 Muon 优化器与 on-policy 蒸馏。

DeepSeek 团队在公告中给出了一个值得注意的自评：
即便有这一轮架构优化，
V4 系列与最前沿闭源模型
（Claude Opus 4.7、GPT-5.4、Gemini 3.1 Pro 一线）
仍存在 3--6 个月的差距。
这是过去两年来 DeepSeek 第一次公开承认
开源旗舰相对闭源前沿不再"接近平价"，
也意味着\textbf{追赶的边际收益正在变小}：
当 V3 用 \$5.6M 复现 GPT-4o 时，开源仅落后 6--12 个月；
V4 在 1.6T 总参数和精细稀疏化之后，
反而把这个 gap 接受为产品方向的一部分。
...
2025 年 4 月 29 日发布的旗舰 MoE 模型，
采用了与 DeepSeek 相似但不同的设计选择：
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{128 个路由专家，Top-8 路由}，
        ...
        专家数减半但每个专家更大。
  \item \textbf{94 层}，使用 GQA（64 Q 头，4 KV 头）。
  \item 支持 128K token 上下文，通过 YaRN 位置编码外推。
  \item \textbf{显式取消共享专家}（与 Qwen2.5-MoE 的不同之处）：
        Qwen3-MoE 技术报告明确说明放弃了 Qwen2.5-MoE 沿用的共享专家设计，
        改用\textbf{全局 batch 负载均衡损失}（global-batch load balancing loss）
        鼓励专家专精化。
\end{itemize}

Qwen3 同系列还有 \textbf{Qwen3-30B-A3B}（30B 总参数 / 3B 活跃，
48 层、32 Q 头 / 4 KV 头、128 专家 Top-8），
是把 Qwen3-235B 的相同 MoE 配比缩小到 1/8 规模的版本，
适合单 GPU 部署。
两个模型都引入了\textbf{思考预算}（thinking budget）机制，
允许用户在调用时显式控制推理链的长度。
...
K2 的训练稳定性来自一个独立公布的优化器细节
\textbf{MuonClip}~\cite{muonClipKimiK22025}：
在 Muon 的基础上加梯度裁剪并精调动量项，
让 1.04T MoE 在 \textbf{15.5T token 全程零 loss spike}。
2026 年 4 月 20 日发布的 \textbf{Kimi K2.6}~\cite{kimik262026}
延续了 K2 的 384 专家 + Top-8 + MLA 配置，
继续把 Agent 数据比例和长上下文 SFT 推上一个台阶。
同样使用 Muon 全程训练的还有 Zhipu 的
\textbf{GLM-4.5}~\cite{glm452025}（355B 级 MoE），
它和 Kimi K2 一起把"Muon 是否适合超大规模 MoE"
这个 2024 年的开放问题
回答成了"是、且能稳定到 15T token+"。

Meta 的 \textbf{LLaMA 4} 在 MoE 路线上选择了相反的极端：
Maverick 用 128 个专家 + Top-1 路由，
Scout 是 16 个专家 + Top-1，
靠从 Behemoth（Top-2、288B 活跃）的大规模知识蒸馏
弥补 Top-1 的信息损失。
LLaMA 4 的 MoE 与 \textbf{iRoPE} 注意力（每 4 层 1 层 NoPE 全局注意力）耦合，
是首个在生产模型中把 MoE 与 NoPE 全局注意力
结合到工业级长上下文（10M）的尝试，
也因此成为"Top-1 路由 + 蒸馏"路线的旗舰参照。

\subsection{Mistral Large 3：Mixtral 之后的 MoE 回归}
\label{sec:mistral-large3}

2025 年 12 月 2 日，Mistral AI 发布了
\textbf{Mistral Large 3}~\cite{mistralLarge3}：
\textbf{675B 总参数 / 41B 活跃参数}的细粒度 MoE 模型，
是 Mistral 自 Mixtral 8x22B（2024 年 4 月）之后
\textbf{首个 MoE 旗舰}，
也是欧洲玩家在中国 MoE 浪潮中的回应。

Large 3 的架构与配套生态有几个值得记住的点：
\begin{itemize}[noitemsep]
  \item \textbf{激活率 6.07\%}（41B / 675B）——
        与 DeepSeek-V3 的 5.5\% 同档，
        但比 Qwen3-Next 的 3.75\% 保守。
        Mistral 选择"中等稀疏度"换取
        每 token 更高的活跃容量。
  \item \textbf{原生 256K 上下文}与原生多模态。
  \item 在 3000 块 \textbf{H200} 上从零训练
        （而非从 Mixtral 衍生），
        与 NVIDIA / vLLM / Red Hat 联合发布
        \textbf{NVFP4 量化检查点}：
        可在单块 GB200 NVL72 系统、
        或单节点 8$\times$A100/8$\times$H100 上跑通推理。
  \item NVIDIA 同步交付了 \textbf{TensorRT-LLM Wide Expert Parallelism}
        和 \textbf{Dynamo}（disaggregated prefill/decode 推理框架）支持。
        Wide-EP 的设计目标是在 NVL72
        coherent memory domain 内做高效的专家并行，
        为 Large 3 这类"专家数比 DeepSeek-R1 少一半"的中等专家配置
        也保持 NVLink 全互联的低延迟。
\end{itemize}

Mistral Large 3 的发布在某种意义上确认了一个行业格局：
\textbf{中等稀疏度 + 中等专家数 + 长上下文 + NVFP4 量化}
正在成为"非中国阵营"开源 MoE 的标准配方。
与 DeepSeek-V4 的 1.6T 总参数旗舰相比，
Large 3 选择 675B 是一个明显的"成本/能力"折中——
2025 年下半年开始，开源 MoE 出现了
"中国阵营冲规模、欧美阵营做工程化"的分工。
...
\begin{tabular}{lccccc}
  ...
  Qwen3-235B-A22B & 235B & 22B & 128 & 8 & 无$^\ddagger$ \\
  Qwen3-30B-A3B & 30B & 3B & 128 & 8 & 无$^\ddagger$ \\
  Kimi K2 & 1.04T & 32B & 384 & 8 & 有 \\
  Kimi K2.6 & $\sim$1T & $\sim$32B & 384 & 8 & 有 \\
  GLM-4.5 & $\sim$355B & -- & -- & -- & -- \\
  Mistral Large 3 & 675B & 41B & -- & -- & 有 \\
  DeepSeek V4-Flash & 284B & 13B & -- & -- & 有 \\
  DeepSeek V4-Pro & 1.6T & 49B & -- & -- & 有 \\
  \bottomrule
\end{tabular}
\end{center}

\small $^\dagger$\,LLaMA 4 未明确公开共享专家设计。
$^\ddagger$\,Qwen3-MoE 技术报告显式说明放弃了 Qwen2.5-MoE 的共享专家，
改用 global-batch load balancing loss 鼓励专家专精化。
\end{keybox}
...
\begin{tabular}{llrrll}
  ...
  & 细粒度+共享 & CSA + HCA \\
  DeepSeek V4-Flash & 2026-04 & 284B & 13B
  & 细粒度+共享 & 1M context \\
  Kimi K2.6 & 2026-04 & $\sim$1T & $\sim$32B
  & 384（8 act.） & MuonClip \\
...
DeepSeek V4 在 2026 年 4 月把活跃参数定在 49B（V4-Pro）/13B（V4-Flash），
配合 CSA + HCA 双稀疏注意力把 1M 上下文的 KV cache
压到 V3.2 的十分之一。
活跃参数量的选择最终取决于
...
\subsection{3:1 Hybrid Layer Ratio 的独立收敛}
\label{sec:moe-3to1-hybrid}

过去 12 个月最具收敛意味的架构信号
不是某个模型的爆点，
而是\textbf{三家互不相干的团队
独立在 MoE 主干上选择了同一个 3:1 的混合比例}——
每 3 层线性注意力或 SSM 风格层
配 1 层 full attention 层。
...
\begin{tabular}{lllll}
  \toprule
  \rowcolor{figLightGray}
  \textbf{模型} & \textbf{活跃 / 总参数} & \textbf{线性层类型}
  & \textbf{Full attn 类型} & \textbf{长上下文} \\
  \midrule
  Qwen 3.5~\cite{qwen35hybrid2026} & 17B / 397B & GatedDeltaNet
  & full + RoPE & 262K $\to$ 1M (YaRN) \\
  Kimi Linear~\cite{kimilinear2025} & 3B / 48B & KDA (channel-wise gate)
  & MLA & 1M (KV $-75\%$) \\
  DeepSeek V3.2~\cite{deepseekv322025} & 37B / 685B & MLA + DSA
  & full + DSA indexer & 128K $\to$ 1M \\
  \bottomrule
\end{tabular}
\end{center}
...
三家走到同一个比例并非偶然。
在 1M 上下文场景下，
\textbf{full attention 的 KV cache 是主要内存瓶颈}，
线性/SSM 层不需要 KV cache，
所以"线性层占比越高、KV 越省"。
但纯 SSM/纯线性的精确检索能力
被反复证明不足以独立支撑 needle-in-a-haystack 等任务，
"插一层 full attention"是经验上无法绕开的下限。
3:1 因此可以读作：
"在能保住精确检索的前提下，
把线性层占比推到经验可承受的最大值"。

Sebastian Raschka~\cite{raschka2026hybridcommentary} 给出的
2026 年下一步判断是：
\textbf{在 3:1 hybrid 内部把 GatedDeltaNet 替换为 Mamba-3
（Ch02 详述）是当前最有意思的方向}。
Mamba-3 的复值 SSM 与 RoPE 的等价性
意味着这种替换可以保留 Qwen 3.5 / Kimi Linear / V3.2
的所有训练成果，
仅升级线性层的核函数；
而 Mamba-3 在 1.5B 规模上同时优于
Mamba-2 与 Gated DeltaNet 的对比
~\cite{mamba32026,mamba3hybridtogether2026}
也已经把这件事的潜在收益做了量化。

值得对照的是 Mamba-3 论文自身推荐的
"\textbf{5 SSM + 1 NoPE attention}"配比——
这是另一个独立收敛点：
两种 hybrid 都把 full attention 压到 25\%--17\%，
但选择了不同的"剩余位置编码"（RoPE vs NoPE）
和不同的线性核（GatedDeltaNet/MLA/KDA vs Mamba-3）。
2026 年下半年的 frontier 模型
大概率会在 \{3:1, 5:1\} $\times$ \{RoPE, NoPE\}
$\times$ \{GatedDeltaNet, KDA, Mamba-3\}
这个九宫格里完成最终收敛。

\subsection{Expert Upcycling：MoE 的连续扩展}
\label{sec:expert-upcycling}

细粒度专家与共享专家是 MoE 在"如何在固定预算下分配专家"上的两个方向。
2026 年 4 月 Amazon 提出的
\textbf{Expert Upcycling}~\cite{expertUpcycling2026}
开辟了第三个方向——
\textbf{把已有的 $E$-专家 MoE 增量扩展为 $mE$-专家 MoE}。

它的形式化设定值得记住：
扩展后保持 top-$K$ 路由不变，
\textbf{每个 token 的推理成本完全不变}。
新增的 $(m-1)E$ 个专家
只在更多种"路由组合"中被偶然激活，
有效扩大模型的知识容量
而不增加 per-token FLOPs。
论文把扩展前后的 quality gap
分解为两项：
（1）\textbf{capacity term}（新增专家本身的容量上限），
（2）\textbf{initialization term}（新专家从旧专家复制时残留的初始化误差）；
他们用一个\textbf{基于梯度重要性}的 utility-based 选择器
来决定从哪些旧专家上 spawn 新专家，
在受限 continual pretraining（CPT）预算下
把 gap closure 提升 \textbf{3 倍}。

实测层面，把 7B 稠密 upcycle 到 13B 总参数（仍然 7B 活跃），
质量与从头训练 13B MoE 持平，
\textbf{节省 32\% 的 GPU 时}。
这条路径在 2024 年的 Mixtral-from-Mistral upcycle
就已经被工程实践，
但 Expert Upcycling 是第一份给出
"capacity 与 initialization 双 term 分解 + utility-driven spawn"
理论框架的工作，
为 fine-grained MoE / shared expert 这两条
"在固定预算内分配"的方向之外，
增加了"沿时间轴增量扩张"的第三种维度。
...
\subsection{Nemotron 3 Super：工业级 Hybrid MoE 旗舰}
\label{sec:nemotron3super}

NVIDIA 在 2026 年 4 月发布的
\textbf{Nemotron 3 Super}~\cite{nemotron3super2026}
（arXiv:2604.12374）
把"Mamba--Attention hybrid + MoE + LatentMoE + 原生 MTP"
四件套同时塞进同一个旗舰模型，
是迄今最完整的工业级 hybrid MoE 实现。

模型规格上，Super 是 \textbf{120B 总参数 / 12B 活跃}
的 hybrid MoE，
预训练 25T tokens、原生 1M 上下文。
它在三个方向上同时落地了过去一年的关键架构进展：

\begin{itemize}[noitemsep]
  \item \textbf{NVFP4 下预训练}：
        首个旗舰级 MoE 用 NVFP4 跑完整训练，
        把 FP8 时代的训练经济性再压一档
  \item \textbf{LatentMoE 集成}（见前一小节）：
        同时优化 accuracy/FLOP 与 accuracy/param 两条曲线，
        路由发生在低维潜空间
  \item \textbf{原生 MTP（Multi-Token Prediction）层}：
        训练时直接预测多个未来 token，
        把推测解码（speculative decoding）从推理 trick
        提升为预训练的一等任务
\end{itemize}

实测层面，Super 的推理吞吐
\textbf{比 GPT-OSS-120B 快 2.2$\times$、
比 Qwen3.5-122B 快 7.5$\times$}：
吞吐差距的主因正是 hybrid 化把
注意力层 KV cache 砍掉一大块、
MoE 把激活参数压到稠密的 1/10。
配套的小弟 \textbf{Nemotron 3 Nano}（30B-A3B）
是 52 层、13 个 macro block 的 hybrid，
每个 macro block 由若干 Mamba-2 子块、一个 MoE 子块、
少量 GQA 层构成，
完整复用 Super 的训练管线。

Super 的意义不在某一项创新，
而在于\textbf{它证明了 2025--2026 年涌现的所有关键 MoE/hybrid 进展
可以在同一个 25T token、120B 总参数级的旗舰里同时落地}：
DeepSeek-V3 是"工业级 MoE 旗舰"的早期参考，
Nemotron 3 Super 现在是"工业级 hybrid MoE 旗舰"的参考。
...
% NEW REF: deepseekv4preview2026 = DeepSeek-AI. "DeepSeek V4 Preview: CSA + HCA at 1.6T/49B and 284B/13B." DeepSeek API release notes, 2026-04-24.
% NEW REF: nemotron3super2026 = NVIDIA. "Nemotron 3 Super: Hybrid Mamba-Attention-MoE." arXiv:2604.12374, 2026.
% NEW REF: expertUpcycling2026 = Amazon Research. "Expert Upcycling: Continual Scaling of MoE Models." arXiv:2604.19835, 2026.
% NEW REF: kimilinear2025 = Moonshot AI. "Kimi Linear: Channel-wise Forget-Gate Linear Attention with MLA." 2025-10.
% NEW REF: deepseekv322025 = DeepSeek-AI. "DeepSeek V3.2: MoE + MLA + DSA at 685B." 2025-12.
% NEW REF: qwen35hybrid2026 = Qwen Team. "Qwen 3.5 Technical Report: GatedDeltaNet-Attention Hybrid MoE." 2026-02.
% NEW REF: kimik262026 = Moonshot AI. "Kimi K2.6 Release Notes." 2026-04-20.
% NEW REF: glm452025 = Zhipu AI. "GLM-4.5: 355B MoE Trained with Muon." 2025.
% NEW REF: muonClipKimiK22025 = Moonshot AI. "MuonClip: Stable Optimizer for Trillion-Parameter MoE." Kimi K2 Technical Report, 2025.
% NEW REF: raschka2026hybridcommentary = Sebastian Raschka. "The 3:1 Hybrid Layer Convergence." Ahead of AI Newsletter, 2026.
% NEW REF: mamba32026 = Albert Gu, Tri Dao et al. "Mamba-3: A Reasoning-Inspired SSM." ICLR 2026 Oral. arXiv:2603.15569.
% NEW REF: mamba3hybridtogether2026 = Together AI. "Mamba-3: Beyond the Limits of Transformers." Blog, 2026-03-17.
%% --- ADDED 2026-04-26 (triple-check pass, agent-A) ---
% NEW REF: mistralLarge3 = Mistral AI. "Mistral Large 3: Granular MoE at 675B/41B." 2025-12-02. https://docs.mistral.ai/models/mistral-large-3-25-12
