...
\begin{tikzpicture}[
  ...
  arr/.style={
    -{Stealth[length=2.5pt,width=2.5pt]}, line width=0.6pt, figGray!60
  },
  brace/.style={
...
% --- Vertical progression arrows (centered on merge pair) ---
\draw[arr] ($(r1d.south)!0.5!(r1e.south) + (0, -7mm)$) -- ++(0, -5mm);
\draw[arr] ($(r2d.south)!0.5!(r2e.south) + (0, -7mm)$) -- ++(0, -5mm);

\end{tikzpicture}
\caption{BPE merge operations on “newest.” Each round merges the highest-frequency adjacent pair:
\texttt{e\,+\,s} $\to$ \texttt{es} (round~1),
\texttt{es\,+\,t} $\to$ \texttt{est} (round~2),
\texttt{est\,+\,\_} $\to$ \texttt{est\_} (round~3).
Orange tokens mark the merged pair; the green token shows the final compressed result.}
\label{fig:bpe-merge}
\end{figure}
