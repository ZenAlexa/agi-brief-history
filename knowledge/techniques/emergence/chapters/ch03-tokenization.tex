...
韩语虽然使用了空格，但一个韩语「词」（어절）通常由词根加多个助词组成（类似日语的活用形式），一个韩语「词」在语义上可能对应英语的一个完整短语。德语则以其复合词著称——``Donaudampfschifffahrtsgesellschaftskapitän''（多瑙河蒸汽船运输公司船长）在德语中是一个合法的词，在词级别词表中需要作为独立条目收录。
...
\begin{tikzpicture}[
  ...
  merged/.style={
    rectangle, rounded corners=2pt,
    draw=figOrange, line width=1pt,
    fill=figOrange!10,
    minimum width=12mm, minimum height=8mm,
    inner sep=4pt, font=\small\ttfamily\bfseries,
    text=figOrange
  },
  result/.style={
    rectangle, rounded corners=2pt,
    draw=figGreen, line width=1pt,
    fill=figGreen!10,
  ...
  roundlabel/.style={
    font=\footnotesize\bfseries, text=figGray
  },
  arr/.style={
    -{Stealth[length=2.5mm]}, line width=0.6pt, figGray
  },
  mergearr/.style={
    -{Stealth[length=2.5mm]}, line width=0.8pt, figOrange, dashed
  }
]

% Round 1
\node[roundlabel] (r1) at (0, 0) {Round 1};
\node[tok, right=8mm of r1] (r1a) {n};
\node[tok, right=2mm of r1a] (r1b) {e};
\node[tok, right=2mm of r1b] (r1c) {w};
\node[merged, right=2mm of r1c] (r1d) {e};
\node[merged, right=2mm of r1d] (r1e) {s};
\node[tok, right=2mm of r1e] (r1f) {t};
\node[tok, right=2mm of r1f] (r1g) {\_};

% Merge indicator for round 1
\draw[mergearr] (r1d.south) -- ++(0, -3mm) -| (r1e.south);
\node[font=\tiny\color{figOrange}, below=5mm of $(r1d)!0.5!(r1e)$] {freq=9};

% Round 2
\node[roundlabel] at (0, -18mm) (r2) {Round 2};
\node[tok, right=8mm of r2] (r2a) {n};
\node[tok, right=2mm of r2a] (r2b) {e};
\node[tok, right=2mm of r2b] (r2c) {w};
\node[merged, right=2mm of r2c] (r2d) {es};
\node[merged, right=2mm of r2d] (r2e) {t};
\node[tok, right=2mm of r2e] (r2f) {\_};

\draw[mergearr] (r2d.south) -- ++(0, -3mm) -| (r2e.south);
\node[font=\tiny\color{figOrange}, below=5mm of $(r2d)!0.5!(r2e)$] {freq=9};

% Round 3
\node[roundlabel] at (0, -36mm) (r3) {Round 3};
\node[tok, right=8mm of r3] (r3a) {n};
\node[tok, right=2mm of r3a] (r3b) {e};
\node[tok, right=2mm of r3b] (r3c) {w};
\node[result, right=2mm of r3c] (r3d) {est\_};

% Result label
\node[font=\scriptsize\color{figGreen}, right=6mm of r3d, align=left] {``newest'' $\to$ [\texttt{n}, \texttt{e}, \texttt{w}, \texttt{est\_}]};

% Vertical arrows between rounds
\draw[arr] ($(r1g.south)+(0,-1mm)$) -- ++(0, -4.5mm);
\draw[arr] ($(r2f.south)+(0,-1mm)$) -- ++(0, -4.5mm);

\end{tikzpicture}
\caption{BPE 合并操作的可视化。以 ``newest'' 为例，展示三轮合并如何将字符序列逐步压缩：\texttt{e+s} $\to$ \texttt{es}，\texttt{es+t} $\to$ \texttt{est}，\texttt{est+\_} $\to$ \texttt{est\_}。橙色标记当前轮被合并的 token 对，绿色标记最终结果。}
\label{fig:bpe-merge}
\end{figure}
...
最终结果：\texttt{[low, est\_]}。模型从未见过 ``lowest'' 这个词，但 BPE 成功地将它切分为两个有意义的子词——``low'' 和 ``est\_''（表示最高级后缀）。这正是 BPE 处理 OOV 问题的优雅之处：通过组合已知的子词片段来表示未知的词。
...
\textbf{词表去重与规范化}。训练完成的词表中可能包含视觉上相同但 Unicode 编码不同的 token（如 ``ﬁ''(U+FB01, Latin Small Ligature FI) 和 ``fi''(U+0066 U+0069)）。工业级实现通常会对词表做 Unicode NFC 规范化，合并这些重复项——否则模型可能对同一个视觉符号学到完全不同的嵌入。
...
\textbf{Fertility（生育率）}——平均每个词被切分为多少个 token。fertility 越接近 1 越好。这是最直接的效率指标，可以针对不同语言单独计算：
...
\textbf{压缩率}——分词后的 token 序列长度与原始字节长度的比值。压缩率越低越好（更少的 token 表示相同内容）。这与 fertility 相关但不同——fertility 以「词」为基准，压缩率以「字节」为基准：

\begin{equation}
  \text{compression ratio} = \frac{\text{token 序列长度}}{\text{原始 UTF-8 字节长度}}
\end{equation}

\textbf{词表覆盖率}——训练语料中不同 token 被使用的比例。如果词表中 20\% 的 token 在训练过程中从未出现，这些 token 就是浪费的词表空间。理想情况下，每个 token 至少在训练过程中出现 $10^4$ 次以上。

\textbf{分词一致性}——语义相似的词是否产生结构相似的分词结果。例如，``playing''、``played''、``plays'' 是否都共享 ``play'' 这个子词？好的分词器应该在形态学变体之间保持一致的子词结构。
...
更激进的方案是\textbf{持续学习分词器}——分词器在模型的整个生命周期中持续更新。当模型通过 continual pre-training 学习新知识时，分词器同步更新词表，将新领域的高频模式纳入词表。这种方案的技术挑战在于：词表的变化会导致已有 token 的嵌入失效——需要一种「增量更新」机制，在添加新 token 的同时保持旧 token 嵌入的稳定性。
...
2025--2026 年 Agentic AI 的兴起给分词带来了新的考量。在 Agent 场景中，模型不仅处理自然语言，还需要与结构化的工具输入/输出交互——JSON 格式的函数调用、API 返回的结构化数据、文件系统路径、URL 等。
