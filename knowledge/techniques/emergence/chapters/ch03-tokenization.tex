...
Qwen3.6-35B-A3B (2026) & 151,643 & tiktoken 风格 & 沿用 Qwen3 词表，强化代码 agent \\
DeepSeek-V4 (2026) & 128,000  & BBPE          & 沿用 V3 词表，新增上下文构造 token \\
\bottomrule
\end{tabular}
\end{center}
...
Kreitner 等人（2025）的 \textbf{BitTokens}~\cite{kreitner2025bittokens}
更进一步：他们指出此前的单 token 数字编码方案
（Golkar 等人 2023 年提出的 xVal、
Zhou 等人 2025 年提出的 FoNE）
在数值范围与精度上各有限制，
转而用 IEEE 754 浮点数的二进制位
直接构造单 token 嵌入，
让小模型在基础算术任务上达到近乎完美的准确率。
这条路线把「分词适配数字结构」
从 split-digits 的字符级粒度
推进到了「一个浮点数 = 一个 token」
的极端形态。
% NEW REF: kreitner2025bittokens = Kreitner, Linus and Hager, Paul and Mengedoht, Jonathan and Kaissis, Georgios and Rueckert, Daniel and Menten, Martin J. Efficient numeracy in language models through single-token number embeddings. arXiv preprint arXiv:2510.06824, 2025.
...
2026 年 4 月 MediaTek Research（与 King's College London 合作）提出的
...
2026 年的两次重要发布进一步印证了这一收敛。
DeepSeek-V4（2026-04-24，1.6T 总参数 / 49B 激活）
直接\textbf{沿用了 V3 的 128K 词表}~\cite{deepseekv4}，
仅新增少量上下文构造 token，
使得 V3 应用迁移到 V4 时几乎不需改动 prompt；
Qwen3.6-35B-A3B（2026-04-16）
也保持了 Qwen3 系列的 151,643 词表。
这种「跨代际复用分词器」的策略
与 LLaMA 1$\to$LLaMA 3 那种大幅扩展形成对比，
说明在词表已经覆盖目标语言、代码和多模态需求之后，
继续放大词表的边际收益迅速递减——
分词器的演进正从「不断扩张」进入「稳定复用」阶段。
...
%% kreitner2025bittokens = Kreitner, Linus et al. Efficient numeracy in language models through single-token number embeddings. arXiv:2510.06824, 2025.
