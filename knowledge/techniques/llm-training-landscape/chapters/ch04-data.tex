...
\begin{tikzpicture}[
  ...
  stage/.style={
    ...
    inner sep=6pt, align=center, line width=0.8pt
  },
  arrow/.style={-{Stealth[length=3mm]}, thick, figGray},
  label/.style={font=\scriptsize\itshape, figGray, align=center},
]
% Row 1
\node[stage=figBlue] (crawl) {Common Crawl\\(HTML)};
\node[stage=figBlue, right=of crawl] (extract) {文本提取\\(trafilatura)};
\node[stage=figGreen, right=of extract] (lang) {语言识别\\(fastText)};
\node[stage=figOrange, right=of lang] (quality) {质量过滤\\(规则+模型)};

% Row 2
\node[stage=figRed, below=1.8cm of quality] (dedup) {多层去重\\(MinHash+LSH)};
\node[stage=figOrange, left=of dedup] (domain) {领域分类\\(domain tagging)};
\node[stage=figGreen, left=of domain] (mix) {数据配比\\(mixing)};
\node[stage=figBlue, left=of mix] (tokenize) {分词 \& 打包\\(tokenization)};

% Arrows Row 1
\draw[arrow] (crawl) -- (extract);
\draw[arrow] (extract) -- (lang);
\draw[arrow] (lang) -- (quality);

% Arrows connecting rows
\draw[arrow] (quality) -- (dedup);

% Arrows Row 2
\draw[arrow] (dedup) -- (domain);
\draw[arrow] (domain) -- (mix);
\draw[arrow] (mix) -- (tokenize);

% Output
\node[stage=figGray, below=1.2cm of tokenize] (output) {训练数据分片\\(sharded tokens)};
\draw[arrow] (tokenize) -- (output);

% Annotations
\node[label, above=0.1cm of crawl] {数百 PB\\HTML};
\node[label, above=0.1cm of extract] {去除模板\\保留正文};
\node[label, above=0.1cm of lang] {按语言\\过滤/分桶};
\node[label, above=0.1cm of quality] {去除垃圾\\保留精华};
\node[label, below=0.1cm of dedup] {URL+精确\\+模糊+子串};
\node[label, below=0.1cm of domain] {网页/代码\\学术/书籍};
\node[label, below=0.1cm of mix] {按比例\\混合各领域};
\node[label, below=0.1cm of tokenize] {BPE 分词\\按序列长度打包};

% Volume annotations (right side)
\node[label, right=0.3cm of quality] {\textbf{--60\%}};
\node[label, left=0.3cm of dedup] {\textbf{--35\%}};

\end{tikzpicture}
\caption{
  LLM 预训练数据处理流水线全景图。
  从原始 Common Crawl 数据出发，
  经过文本提取、语言识别、质量过滤、多层去重、
  领域分类、数据配比到最终的分词打包。
  每一步都大幅减少数据量：
  质量过滤通常移除约 60\% 的数据，
  去重进一步移除约 35\%。
  最终产出的高质量 token 仅为原始数据的 10--15\%。
}
