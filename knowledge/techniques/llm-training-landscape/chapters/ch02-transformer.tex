...
\begin{tikzpicture}[
  ...
  proj/.style={box, fill=figBlue!15, draw=figBlue!60},
  attn/.style={box, fill=figOrange!15, draw=figOrange!60},
  merge/.style={box, fill=figGreen!15, draw=figGreen!60},
  arr/.style={-{Stealth[length=2.5mm]}, thick},
  darr/.style={-{Stealth[length=2.5mm]}, thick, dashed, figGray},
  lab/.style={font=\scriptsize, figGray},
]
% Input
\node[box, fill=figLightGray, minimum width=65mm] (input) {Input $X \in \mathbb{R}^{n \times d}$};

% Q, K, V projections
\node[proj, above=12mm of input, xshift=-25mm] (wq) {$W^Q$};
\node[proj, above=12mm of input] (wk) {$W^K$};
\node[proj, above=12mm of input, xshift=25mm] (wv) {$W^V$};
\draw[arr] (input.north -| wq) -- (wq);
\draw[arr] (input.north -| wk) -- (wk);
\draw[arr] (input.north -| wv) -- (wv);

% Labels
\node[lab, left=1mm of wq] {Query};
\node[lab, above left=0mm and 1mm of wk] {Key};
\node[lab, right=1mm of wv] {Value};

% Split into heads
\node[lab, above=5mm of wq] (splitq) {\textbf{Split} $\rightarrow h$ heads};
\node[lab, above=5mm of wk] (splitk) {\textbf{Split} $\rightarrow h$ heads};
\node[lab, above=5mm of wv] (splitv) {\textbf{Split} $\rightarrow h$ heads};
\draw[darr] (wq) -- (splitq);
\draw[darr] (wk) -- (splitk);
\draw[darr] (wv) -- (splitv);

% Attention heads
\node[attn, above=18mm of wq] (h1) {Head 1};
\node[attn, above=18mm of wk] (h2) {Head 2};
\node[attn, above=18mm of wv] (hdots) {$\cdots$ Head $h$};
\draw[arr, figBlue] (splitq.north) -- ++(0,4mm) -| (h1.south);
\draw[arr, figBlue] (splitk.north) -- ++(0,4mm) -| (h1.south);
\draw[arr, figBlue] (splitv.north) -- ++(0,4mm) -| (h1.south);

\draw[arr, figOrange] (splitq.north) -- ++(0,4mm) -| (h2.south);
\draw[arr, figOrange] (splitk.north) -- ++(0,4mm) -| (h2.south);
\draw[arr, figOrange] (splitv.north) -- ++(0,4mm) -| (h2.south);

\draw[arr, figGray] (splitq.north) -- ++(0,4mm) -| (hdots.south);
\draw[arr, figGray] (splitk.north) -- ++(0,4mm) -| (hdots.south);
\draw[arr, figGray] (splitv.north) -- ++(0,4mm) -| (hdots.south);

% Head detail annotation
\node[lab, right=8mm of hdots] (detail) {%
  $\mathrm{head}_i = \mathrm{softmax}\!\left(\frac{Q_i K_i^\top}{\sqrt{d_k}}\right) V_i$};

% Concat
\node[merge, above=12mm of h2, minimum width=50mm] (concat)
  {Concat($\mathrm{head}_1, \ldots, \mathrm{head}_h$)};
\draw[arr] (h1) -- (h1 |- concat.south);
\draw[arr] (h2) -- (concat);
\draw[arr] (hdots) -- (hdots |- concat.south);

% Output projection
\node[merge, above=8mm of concat, minimum width=30mm] (wo) {$W^O$ 输出投影};
\draw[arr] (concat) -- (wo);

% Output
\node[box, fill=figLightGray, above=8mm of wo, minimum width=50mm] (output)
  {Output $\in \mathbb{R}^{n \times d}$};
\draw[arr] (wo) -- (output);

% Dimension annotations
\node[lab, left=15mm of h1, text width=20mm, align=right] {每个头维度\\$d_k = d/h$};
\end{tikzpicture}
\caption{多头注意力的数据流。输入 $X$ 经过三个线性投影得到 $Q, K, V$，
然后被分割到 $h$ 个平行的注意力头中独立计算。
每个头在 $d/h$ 维的子空间中计算注意力，
最终通过拼接和输出投影融合为 $d$ 维的输出。
整个过程的参数量与单头注意力相同。}
...
\begin{tikzpicture}[
  ...
  qhead/.style={draw, fill=figBlue!25, minimum width=6mm, minimum height=10mm,
    rounded corners=1pt, font=\tiny, inner sep=1pt},
  kvhead/.style={draw, fill=figOrange!25, minimum width=6mm, minimum height=10mm,
    rounded corners=1pt, font=\tiny, inner sep=1pt},
  latent/.style={draw, fill=figGreen!25, minimum width=28mm, minimum height=10mm,
    rounded corners=1pt, font=\tiny, inner sep=2pt},
  title/.style={font=\small\bfseries, anchor=south},
  brace/.style={decorate, decoration={brace, amplitude=4pt, mirror}},
  conn/.style={-{Stealth[length=1.5mm]}, thin, figGray},
]

% --- MHA ---
\node[title] at (0, 2.8) {MHA};
% Q heads
\foreach \i in {1,...,4} {
  \node[qhead] (mha-q\i) at ({(\i-2.5)*0.9}, 1.5) {Q\i};
}
% KV heads (each independent)
\foreach \i in {1,...,4} {
  \node[kvhead] (mha-kv\i) at ({(\i-2.5)*0.9}, 0) {KV\i};
  \draw[conn] (mha-q\i) -- (mha-kv\i);
}
\node[font=\tiny, figGray] at (0, -0.9) {4 KV heads};

% --- GQA ---
\begin{scope}[xshift=45mm]
\node[title] at (0, 2.8) {GQA};
\foreach \i in {1,...,4} {
  \node[qhead] (gqa-q\i) at ({(\i-2.5)*0.9}, 1.5) {Q\i};
}
% 2 KV heads (shared pairwise)
\node[kvhead, minimum width=14mm] (gqa-kv1) at (-0.45, 0) {KV$_1$};
\node[kvhead, minimum width=14mm] (gqa-kv2) at (0.45, 0) {KV$_2$};
\draw[conn] (gqa-q1) -- (gqa-kv1);
\draw[conn] (gqa-q2) -- (gqa-kv1);
\draw[conn] (gqa-q3) -- (gqa-kv2);
\draw[conn] (gqa-q4) -- (gqa-kv2);
\node[font=\tiny, figGray] at (0, -0.9) {2 KV heads};
\end{scope}

% --- MQA ---
\begin{scope}[xshift=90mm]
\node[title] at (0, 2.8) {MQA};
\foreach \i in {1,...,4} {
  \node[qhead] (mqa-q\i) at ({(\i-2.5)*0.9}, 1.5) {Q\i};
}
% 1 shared KV head
\node[kvhead, minimum width=28mm] (mqa-kv) at (0, 0) {KV (shared)};
\foreach \i in {1,...,4} {
  \draw[conn] (mqa-q\i) -- (mqa-kv);
}
\node[font=\tiny, figGray] at (0, -0.9) {1 KV head};
\end{scope}

% --- MLA ---
\begin{scope}[xshift=135mm]
\node[title] at (0, 2.8) {MLA};
\foreach \i in {1,...,4} {
  \node[qhead] (mla-q\i) at ({(\i-2.5)*0.9}, 1.5) {Q\i};
}
% Latent vector
\node[latent] (mla-c) at (0, 0) {Latent $c$ ($d_c \ll d$)};
\foreach \i in {1,...,4} {
  \draw[conn] (mla-q\i) -- (mla-c);
}
\node[font=\tiny, figGray] at (0, -0.9) {low-rank KV};
\end{scope}

\end{tikzpicture}
\caption{KV 共享策略的演进。从左到右：MHA 中每个查询头有独立的 KV 头；
GQA 中多个查询头共享一个 KV 头；MQA 中所有查询头共享一个 KV 头；
MLA 中所有头共享一个低维潜在向量 $c$，在计算时再投影回 KV 空间。
KV cache 大小从 MHA 的 $O(hd)$ 依次减少到 MLA 的 $O(d_c)$。}
